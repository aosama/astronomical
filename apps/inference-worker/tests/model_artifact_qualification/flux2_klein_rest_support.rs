//! Isolated production supervisor and worker boundary for native FLUX qualification.
...
use astronomical_model_serving::{FLUX2_KLEIN_OFFICIAL_MODEL_ID, FLUX2_KLEIN_OFFICIAL_REVISION};
...
use super::model_artifact_rest_transport::send_http_request;

const READY_ATTEMPT_LIMIT: u8 = 70;

pub(super) struct FluxRestServer {
...
pub(super) async fn launch_flux_rest_server() -> FluxRestServer {
    let isolated_development_home = crate::common::isolated_development_home_from_user_config();
    enable_isolated_qualification_diagnostics(&isolated_development_home);
    ...
    .await
    .expect("the production worker should launch for FLUX qualification");
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("the FLUX qualification listener should bind");
    let server_address = listener
        .local_addr()
        .expect("the FLUX qualification listener should expose its address");
...
impl FluxRestServer {
    pub(super) async fn stop(self) {
        ...
        self.worker_handle
            .shutdown()
            .await
            .expect("the FLUX qualification worker should terminate and be reaped");
    }

    pub(super) fn attribution_log_path(&self) -> PathBuf {
    ...
    pub(super) fn diagnostic_logs(&self) -> String {
        ...
        let Ok(log_entries) = fs::read_dir(logging_directory) else {
            return "qualification log directory was unavailable".to_owned();
        };
...
pub(super) fn assert_image_attribution(rest_server: &FluxRestServer, expected_outcomes: &[&str]) {
    ...
    for (report, expected_outcome) in image_reports.iter().zip(expected_outcomes) {
        ...
        assert_eq!(report["model_revision"], FLUX2_KLEIN_OFFICIAL_REVISION);
        assert_eq!(
    ...
    eprintln!(
        "[flux-qualification] phase=cleanup reports={} allocator_cache_bytes=0",
        image_reports.len()
    );
}

pub(super) async fn wait_for_image_attribution_count(
    ...
    for poll_count in 1..=100_u16 {
        ...
        if report_count >= expected_report_count {
            eprintln!(
                "[flux-qualification] phase={phase_label} reports={report_count} polls={poll_count}"
            );
            return;
        }
...
pub(super) async fn get_status(server_address: SocketAddr) -> serde_json::Value {
...
pub(super) async fn post_image(server_address: SocketAddr, request_body: String) -> String {
    send_http_request(
        server_address,
        format!(
...
fn enable_isolated_qualification_diagnostics(isolated_home: &tempfile::TempDir) {
...
async fn wait_until_ready(server_address: SocketAddr) {
    for readiness_attempt in 1..=READY_ATTEMPT_LIMIT {
        ...
        if status["status"] == "ready" {
            eprintln!("[flux-qualification] phase=worker-ready attempt={readiness_attempt}");
            return;
        }
        eprintln!(
            "[flux-qualification] phase=worker-starting attempt={readiness_attempt}/{READY_ATTEMPT_LIMIT}"
        );
        sleep(Duration::from_secs(1)).await;
    }
    panic!("the FLUX qualification worker did not become ready before the deadline");
}
