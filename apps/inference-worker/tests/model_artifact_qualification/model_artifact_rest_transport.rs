...
pub(super) fn assert_streamed_model_text_mentions_red(streamed_model_text: &str) -> &'static str {
    let normalized_model_text = streamed_model_text.to_lowercase();
    let accepted_red_terms = ["red", "crimson", "scarlet"];
    if let Some(matched_red_term) = accepted_red_terms
        .iter()
        .find(|red_term| normalized_model_text.contains(**red_term))
    {
        return matched_red_term;
    }
    panic!("model output did not identify the synthetic image as red: {streamed_model_text:?}");
}

pub(super) async fn send_http_request(server_address: SocketAddr, request_text: String) -> String {
