...
use super::prompt_processing_chunk_optimizer_comparison::benchmark_command;

const OPTIMIZER_CANDIDATE_MEASUREMENT_PROMPT_TOKENS: usize = 20_000;
const OPTIMIZER_CANDIDATE_MEASUREMENT_TIMEOUT: Duration = Duration::from_secs(115);

#[tokio::test(flavor = "multi_thread")]
#[ignore = "loads Ornith through the configuration-enabled optimizer and observes large candidates"]
async fn should_measure_complete_large_candidates_with_the_configuration_enabled_optimizer() {
    timeout(
        OPTIMIZER_CANDIDATE_MEASUREMENT_TIMEOUT,
        run_configuration_enabled_optimizer_candidate_measurement(),
    )
    .await
    .expect("the optimizer candidate measurement must finish within 115 seconds");
}

async fn run_configuration_enabled_optimizer_candidate_measurement() {
    let configured_model_directory = configured_prefill_qualification_model_directory();
    let optimizer_state_reload_is_required =
        std::env::var_os("ASTRONOMICAL_REQUIRE_PROMPT_PROCESSING_CHUNK_OPTIMIZER_STATE_RELOAD")
            .is_some();
    let mut exact_prompt_content = build_prefill_qualification_prompt(
        &configured_model_directory,
        OPTIMIZER_CANDIDATE_MEASUREMENT_PROMPT_TOKENS,
    );
    if let Ok(prompt_variant) =
        std::env::var("ASTRONOMICAL_PROMPT_PROCESSING_CHUNK_OPTIMIZER_PROMPT_VARIANT")
    {
    ...
    let mut stream_receiver = worker_handle
        ...
        .await
        .expect("the optimizer candidate measurement should start");
    ...
    while let Some(stream_event) = stream_receiver.recv().await {
        match stream_event {
            ChatGenerationStreamEvent::PrefillProgress {
                ...
                assert!(
                    total_tokens >= OPTIMIZER_CANDIDATE_MEASUREMENT_PROMPT_TOKENS as u32,
                    "the optimizer candidate measurement prompt is shorter than requested"
                );
                ...
                eprintln!(
                    "[prompt-processing-chunk-optimizer:optimizer-candidates] processed={processed_tokens}/{total_tokens} completed_prefill_chunk_tokens={completed_prefill_chunk_tokens:?}"
                );
            }
            ChatGenerationStreamEvent::Completed {
                ...
                assert!(
                    prompt_token_count >= OPTIMIZER_CANDIDATE_MEASUREMENT_PROMPT_TOKENS as u32,
                    "the completed optimizer measurement reports fewer tokens than requested"
                );
            ...
            ChatGenerationStreamEvent::Failed { reason } => {
                panic!("the optimizer candidate measurement failed: {reason:?}");
            }
            ChatGenerationStreamEvent::Error(error_code) => {
                panic!("the optimizer candidate measurement stream failed: {error_code:?}");
            }
    ...
    assert_persisted_chunk_measurements(
        &prepared_prefill_qualification_worker.optimizer_state_file_path(),
    );
    worker_handle
        .shutdown()
        .await
        .expect("the optimizer candidate measurement worker should terminate");
}

pub(super) fn assert_cumulative_latency_optimizer_measurements(
        ...
        prefill_measurements
            ...
            .all(|prefill_chunck_measurement| {
                prefill_chunck_measurement.actual_prefill_chunck_tokens
                    == prefill_chunck_measurement.reported_completed_prefill_chunk_tokens
            }),
        "the progress protocol should report actual completed advancement without calling it the selected request"
    );
}

fn assert_persisted_chunk_measurements(optimizer_state_file_path: &Path) {
    ...
    let persisted_measurement_count = execution_profile_buckets
        ...
        .filter_map(|candidate_statistics| candidate_statistics["measurements"].as_array())
        .map(Vec::len)
        .sum::<usize>();
    assert!(
        persisted_measurement_count > 0,
        "optimizer state should retain completed chunk measurements"
    );
}
