...
use super::prompt_processing_chunk_optimizer_candidate_measurement::assert_cumulative_latency_optimizer_measurements;
...
async fn run_prefill_benchmark(
    ...
    eprintln!("[prompt-processing-chunk-optimizer:{benchmark_label}] launching worker; ETA <= 70s");
        ...
        match stream_event {
            ChatGenerationStreamEvent::PrefillProgress {
                ...
                if processed_tokens >= next_progress_token_count {
                    ...
                    eprintln!(
                        "[prompt-processing-chunk-optimizer:{benchmark_label}] processed={processed_tokens}/{total_tokens} native_elapsed_ms={elapsed_millis} latest_forward_ms={} latest_allocator_clear_and_telemetry_ms={} mlx_active_bytes={} mlx_allocator_cache_bytes={} mlx_peak_bytes={}",
                        latest_prefill_chunck_measurement.forward_prefill_chunk_elapsed_millis,
    ...
    eprintln!("[prompt-processing-chunk-optimizer-report] {benchmark_report}");
...
async fn record_first_output_footprint(
    ...
    *first_output_footprint = Some(
    ...
    eprintln!("[prompt-processing-chunk-optimizer:{benchmark_label}] first output received");
}
