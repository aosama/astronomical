...
pub(crate) const MAXIMUM_OUTPUT_TOKEN_COUNT: u16 = 2_048;
pub(crate) const THINKING_BUDGET_TOKEN_COUNT: u16 = 256;
pub(crate) const PROMPT_CACHE_MAXIMUM_SIZE_GB: u64 = 50;
const ACCEPTANCE_SAMPLING_SEED: u64 = 42;

const ROMEO_AND_JULIET_SOURCE: &str =
    include_str!("../fixtures/model_metrics_5000_romeo_and_juliet_words.txt");

pub(crate) struct PreparedRomeoAndJulietPrompt {
...
pub(crate) struct StreamedAssistantResponse {
    pub(crate) assistant_content: String,
    pub(crate) prompt_token_count: u64,
    pub(crate) streamed_output_character_count: usize,
}

pub(crate) fn prepare_cacheable_romeo_and_juliet_prompt(
...
pub(crate) async fn send_streaming_chat_request(
    ...
    while let Some(stream_item) = streamed_chat_completion.next().await {
        ...
        if received_chunk_count.is_multiple_of(128) {
            eprintln!(
                "{acceptance_log_prefix} status=progress phase={request_phase_name}_stream received_chunks={received_chunk_count} output_characters={streamed_output_character_count}"
            );
        }
    }
    assert!(
    ...
    eprintln!(
        "{acceptance_log_prefix} status=progress phase={request_phase_name}_complete received_chunks={received_chunk_count} output_characters={streamed_output_character_count}"
    );
    StreamedAssistantResponse {
        assistant_content,
...
pub(crate) fn user_message(content: &str) -> Value {
    json!({ "role": "user", "content": content })
}

pub(crate) fn assistant_message(content: &str) -> Value {
...
pub(crate) fn required_u64(json_document: &Value, field_name: &str) -> u64 {
    json_document[field_name].as_u64().unwrap_or_else(|| {
        panic!("the cache stats response must contain {field_name}: {json_document}")
    })
}

pub(crate) fn read_performance_records(performance_log_directory: &Path) -> Vec<Value> {
...
pub(crate) fn write_cache_pressure_worker_config(
    ...
    if let Some(maximum_mlx_memory_bytes) = maximum_mlx_memory_bytes {
        configuration_document["maximum_mlx_memory_gb"] =
            json!(maximum_mlx_memory_bytes / 1_000_000_000);
    }
    fs::write(
