...
#[ignore = "launches the production REST server and publishes then restores a Romeo and Juliet cache"]
async fn should_publish_and_restore_the_romeo_and_juliet_cache_through_rest() {
    timeout(
        E2E_TIMEOUT,
...
async fn run_persistent_prompt_cache_memory_rest_journey(
    ...
    let acceptance_log_prefix = "[persistent-prompt-cache-rest]";
    let model_directory =
    ...
    if expected_warm_request_expert_memory_mode.is_some() {
        ...
        assert_eq!(
            ...
            "the long warm request must remain resident through ordinary initial admission so this acceptance isolates restore pressure: {warm_cache_diagnostics}"
        );
        assert!(
...
async fn wait_for_expert_memory_mode(
    ...
    loop {
        ...
        if observed_expert_memory_mode == Some(expected_expert_memory_mode) {
            eprintln!(
                "{acceptance_log_prefix} status=progress phase=warm_request_mode expert_memory_mode={expected_expert_memory_mode}"
            );
            return;
        }
        assert!(
...
fn assert_expert_memory_mode(
    ...
    let observed_expert_memory_mode = status_document["expert_memory_mode"].as_str();
    eprintln!(
        "{acceptance_log_prefix} status=progress phase={status_phase_name} expert_memory_mode={observed_expert_memory_mode:?}"
    );
