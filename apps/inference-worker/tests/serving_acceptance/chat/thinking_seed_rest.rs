//! Proves config-gated Qwen reasoning seeding across both public streaming REST APIs.
...
fn write_thinking_seed_acceptance_state(
    isolated_worker_home: &Path,
    model_id: &str,
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&configuration_document)
            .expect("the thinking-seed acceptance configuration should serialize"),
    )
    .expect("the thinking-seed acceptance configuration should be written");
    fs::write(
