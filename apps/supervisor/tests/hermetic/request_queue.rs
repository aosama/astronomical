...
/// When a queued request's future is dropped (simulating HTTP client
/// disconnect while waiting in the queue), the queue slot should be
/// released immediately, allowing another request to enter the queue.
///
/// This test verifies that dropping a queued request doesn't leak its
/// queue permit: after dropping, a new request can enter the queue
/// (which would fail with CapacityUnavailable if the permit leaked).
#[tokio::test]
async fn should_release_queue_slot_when_queued_request_is_dropped() {
    let worker_executor = launch_fixture().await;
    ...
    {
        let queued_future = worker_executor
            .start_chat_generation(command_with_request_id("astronomical/test-worker", 2));
        // Drop the future — this simulates the HTTP client disconnecting
        // while waiting in the queue. Both the queue permit and the
        // in-progress acquire_owned() should be released.
        drop(queued_future);
    }

    // Give the dropped future time to release its permits
    sleep(Duration::from_millis(50)).await;

    // A new request should be able to enter the queue. The key assertion is
    // that the previous drop didn't leak the queue permit.
    // If the queue permit leaked, this would return CapacityUnavailable.
    // If it didn't leak, this will queue (and block until Request A completes).
    //
    // We use a timeout so the test doesn't hang forever. Since Request A is
    // still active, this request will block on acquire_owned(), so we can't
    // get an Ok result — we just verify it doesn't return CapacityUnavailable.
    let queue_result = timeout(
        Duration::from_millis(200),
        worker_executor.start_chat_generation(minimal_command()),
    )
    .await;

    match queue_result {
        Ok(Ok(_)) => {
            // Request started immediately (Request A must have completed or been cancelled)
            // This is fine — the queue slot was available.
        }
        Ok(Err(GenerationStartError::CapacityUnavailable)) => {
            panic!("queue permit was leaked — dropped queued request did not release its slot");
        }
        Ok(Err(GenerationStartError::ModelLoadFailed {
            model_load_failure_reason,
        })) => {
            panic!(
                "the already loaded test model should not fail to load: {model_load_failure_reason}"
        ...
        Err(_) => {
            // Timeout: Request A is still active so the new request is queued
            // waiting for the active slot. This is EXPECTED behavior — the
            // important thing is that we didn't get CapacityUnavailable,
            // which would indicate a leaked queue permit.
            // The request is properly queued and waiting.
        }
    }

    // Clean up: cancel Request A
    drop(stream_a);
    sleep(Duration::from_secs(2)).await;
