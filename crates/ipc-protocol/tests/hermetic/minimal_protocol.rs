...
async fn should_round_trip_prefill_progress_event() {
    ...
    worker_writer
        .send_event(&worker_event)
        .await
        .expect("a progress event should be written");

    assert_eq!(
        supervisor_reader
            .next_event()
            .await
            .expect("the progress event frame should decode"),
        Some(worker_event)
    );
}
...
async fn should_round_trip_prefill_progress_before_a_chunk_measurement() {
    ...
    worker_writer
        .send_event(&worker_event)
        .await
        .expect("an initial prefill event should be written");

    assert_eq!(
        supervisor_reader
            .next_event()
            .await
            .expect("the initial prefill event frame should decode"),
        Some(worker_event)
    );
}
