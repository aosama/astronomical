...
pub enum DecoderCacheLayoutError {
    ...
    #[error(
        "decoder-cache layer {layer_index} sequence tensor {tensor_role_name} has no sequence axis"
    )]
    ...
    #[error(
        "decoder-cache layer {layer_index} tensor {tensor_role_name} has sequence axis {sequence_axis} outside rank {tensor_rank}"
    )]
    ...
    #[error(
        "decoder-cache layer {layer_index} sequence tensor {tensor_role_name} must use zero for its dynamic sequence dimension"
    )]
    ...
    #[error(
        "decoder-cache layer {layer_index} boundary tensor {tensor_role_name} must not have a sequence axis"
    )]
    ...
    #[error(
        "decoder-cache layer {layer_index} boundary tensor {tensor_role_name} must not have a dynamic dimension"
    )]
    ...
    #[error("decoder-cache layer {layer_index} tensor {tensor_role_name} has zero rank")]
    ZeroTensorRank {
        layer_index: usize,
        tensor_role_name: String,
    },
    #[error("decoder-cache layer {layer_index} repeats tensor role {tensor_role_name}")]
    DuplicateTensorRole {
        layer_index: usize,
        tensor_role_name: String,
    },
    #[error("decoder-cache tensor {tensor_role_name} has invalid payload geometry: {description}")]
    InvalidTensorPayloadGeometry {
        tensor_role_name: String,
        description: &'static str,
    },
    #[error("decoder-cache tensor {tensor_role_name} payload byte count overflowed")]
    TensorPayloadByteCountOverflow { tensor_role_name: String },
