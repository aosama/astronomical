//! The `MlxInferenceExecution` engine implementation for the owner-thread
//! K2 Horizon MoVA execution: load, generation start, decode, cache
//! maintenance, and live memory-limit adjustment. Split from the execution
//! state vocabulary in the parent so each file stays inside the
//! source-size budget; as a child module it reaches the parent's private
//! fields directly.
...
impl MlxInferenceExecution for K2HorizonMoVAInferenceExecution {
    ...
    fn load(&mut self) -> Result<EngineLoadResult, InferenceEngineError> {
        ...
        .map_err(super::super::prefill::execution_error)?;
        ...
        if let Some(prompt_cache_disk_store_config) = pending.prompt_cache_disk_store_config {
            ...
            self.persistent_prompt_cache =
                Some(super::super::prompt_cache::open_prompt_cache_store(
                    pending.effective_mlx_memory_ceiling_bytes,
                    &model.config,
        ...
        super::super::memory::reject_unenacted_expert_streaming(
            self.model
                .as_ref()
                .ok_or_else(|| InferenceEngineError::Fatal {
                    reason: "K2 Horizon MoVA model is not loaded".to_owned(),
    ...
    fn start_generation(
        ...
        if let Some(persistent_prompt_cache) = self.persistent_prompt_cache.clone() {
            let lookup_result = super::super::prompt_cache::lookup_prompt_prefix(
            ...
            if restored_token_count > 0 {
                let (restored_block_key, restored_tokens) =
                    super::super::prompt_cache::restore_prompt_prefix(
                        &model.runtime,
                        &persistent_prompt_cache,
                        &prompt_token_ids,
    ...
    fn decode_next_token(
        ...
        if has_remaining_prompt {
            let mut prefill_progress = {
                ...
                super::super::prefill::prefill_next_chunk(
                    model,
                    active,
    ...
    fn inject_input_tokens(
        ...
        active.performance_attribution = performance_attribution;
        forward_result.map_err(super::super::prefill::execution_error)?;
        Ok(())
    }
