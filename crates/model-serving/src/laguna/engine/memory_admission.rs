...
use crate::{
    ...
    PerformanceAttribution, PerformanceOperation, persistent_context_restore_workspace_bytes,
};
...
pub(super) fn admit_generation_context(
    ...
    for _admission_attempt in 0..3 {
        ...
        let decoder_memory_projection = match decoder_state {
            Some(decoder_state) => {
                decoder_state.projected_forward_memory(model.contract(), context_growth_token_count)
            }
        ...
        let prompt_cache_restore_workspace_bytes = if restored_prompt_prefix_token_count == 0 {
            0
        } else {
            persistent_context_restore_workspace_bytes(
                decoder_cache_layout
                    ...
                    .map_err(|layout_error| InferenceEngineError::InvalidRequest {
                        reason: format!(
                            "Laguna prompt-cache sequence geometry is invalid: {layout_error}"
                        ),
                    })?,
                restored_prompt_prefix_token_count,
            )
            .and_then(|sequence_workspace_bytes| {
                decoder_cache_layout
                    .boundary_snapshot_payload_byte_count()
                    .ok()
                    .and_then(|boundary_bytes| sequence_workspace_bytes.checked_add(boundary_bytes))
            })
            .ok_or(InferenceEngineError::InvalidRequest {
                reason: "Laguna prompt-cache restoration workspace overflowed".to_owned(),
            })?
        };
