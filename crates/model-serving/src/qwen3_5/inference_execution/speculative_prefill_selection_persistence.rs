...
impl Qwen3_5EngineState {
    ...
    pub(super) fn persist_speculative_prefill_selection(
        ...
        match draft_scoring_outcome {
            Ok((absolute_selected_token_positions, mut draft_scoring_outcome)) => {
                    ...
                    )
                {
                    let selected_token_position_values = absolute_selected_token_positions
                        .iter()
                        .filter_map(|selected_token_position| {
                            u32::try_from(*selected_token_position).ok()
                        })
                        .collect::<Vec<_>>();
                    if selected_token_position_values.len()
                        == absolute_selected_token_positions.len()
                        && let Ok(selected_token_position_count_i32) =
                            i32::try_from(selected_token_position_values.len())
                        && let Ok(selected_token_positions_on_gpu) =
                            draft_model.runtime().array_from_u32(
                                &selected_token_position_values,
                                &[selected_token_position_count_i32],
                            )
                        && let Err(selection_save_error) = active_request
                    ...
                    {
                        tracing::warn!(
                            error = %selection_save_error,
                            "failed to persist speculative-prefill selection; continuing with in-memory result"
                        );
                    }
                }
                if !is_visual_speculative_prefill_request {
            ...
            Err(Qwen3_5ExecutionError::Runtime(
                mlx_runtime_error @ MlxRuntimeError::ActiveMemoryLimitExceeded { .. },
            )) => {
                return Err(Qwen3_5ExecutionError::Runtime(mlx_runtime_error).into());
            }
