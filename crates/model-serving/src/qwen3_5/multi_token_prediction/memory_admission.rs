...
/// Exact operation-local logits, hidden rows, and decision vectors for one verification window.
///
/// Boundary snapshots are excluded so admission can charge them once as a separate owner.
/// Proposal logits, verifier logits, hidden rows, and decision ids share one lazy graph and
/// therefore coexist at the single completion boundary.
pub fn qwen3_5_mtp_verification_transient_array_bytes(
    depth: MtpDraftDepth,
...
#[must_use]
pub fn qwen3_5_mtp_memory_admission(
