...
use super::Qwen3_5MoEMtpArtifactCapability;
use super::tensor_spec::qwen3_5_moe_language_tensor_profiles;
use super::vision_tensor_spec::qwen3_5_moe_vision_tensor_profiles;
...
use super::{
    OptiQMetadata, OptiQMetadataError, Qwen3_5MoEConfig, Qwen3_5MoEConfigError,
    Qwen3_5MoEVisionConfig,
};
use super::{Qwen3_5MoEArtifactError, Qwen3_5MoEShardIndex};

/// Validates the complete Qwen3.5-MoE artifact before any native allocation.
///
...
pub struct Qwen3_5MoEArtifactValidator;

impl Qwen3_5MoEArtifactValidator {
    /// Creates the validator for Qwen3.5-MoE model artifacts.
    #[must_use]
    ...
    pub fn validate(
    ...
    ) -> Result<ValidatedQwen3_5MoEArtifact, Qwen3_5MoEArtifactValidationError> {
        ...
        let vision_config = Qwen3_5MoEVisionConfig::from_optional_json_bytes(config_bytes)?;
        ...
        let shard_tensor_names =
            Qwen3_5MoEShardIndex::extract_language_tensor_names_from_json(&shard_index_bytes)?;
        config.resolve_unquantized_gates_from_shard_index(&shard_tensor_names);
        let language_tensor_profiles = qwen3_5_moe_language_tensor_profiles(&config);
        let shard_index =
            Qwen3_5MoEShardIndex::from_json_bytes(&shard_index_bytes, &language_tensor_profiles)?;
        let mtp_artifact_capability =
            Qwen3_5MoEMtpArtifactCapability::from_shard_index(&config, &shard_index);
        ...
        let vision_tensor_profiles = vision_config
            .as_ref()
            .map(qwen3_5_moe_vision_tensor_profiles)
            .unwrap_or_default();
        ...
        Ok(ValidatedQwen3_5MoEArtifact {
            config,
            vision_config,
            required_files,
            shard_index,
...
/// A cause-preserving failure while validating the complete Qwen3.5-MoE artifact.
#[derive(Debug, Error)]
pub enum Qwen3_5MoEArtifactValidationError {
    #[error("Qwen3.5-MoE file or safetensors validation failed")]
    Artifact(#[from] ArtifactValidationError),
    #[error("Qwen3.5-MoE config validation failed")]
    Config(#[from] Qwen3_5MoEConfigError),
    #[error("Qwen3.5-MoE OptiQ metadata validation failed")]
    OptiQMetadata(#[from] OptiQMetadataError),
    #[error("Qwen3.5-MoE shard-index validation failed")]
    Qwen3_5MoEShardIndex(#[from] Qwen3_5MoEArtifactError),
}

/// Descriptor-backed validated ownership of the complete Qwen3.5-MoE artifact.
#[derive(Debug)]
pub struct ValidatedQwen3_5MoEArtifact {
    config: Qwen3_5MoEConfig,
    vision_config: Option<Qwen3_5MoEVisionConfig>,
...
impl ValidatedQwen3_5MoEArtifact {
    #[must_use]
    pub const fn config(&self) -> &Qwen3_5MoEConfig {
        &self.config
    }

    /// Returns the validated Qwen3.5-MoE vision configuration.
    #[must_use]
    pub const fn vision_config(&self) -> Option<&Qwen3_5MoEVisionConfig> {
    ...
    #[must_use]
    pub const fn shard_index(&self) -> &Qwen3_5MoEShardIndex {
        &self.shard_index
    }
    ...
    #[must_use]
    pub const fn mtp_artifact_capability(&self) -> &Qwen3_5MoEMtpArtifactCapability {
