...
use super::{Qwen3_5MoEArtifactError, Qwen3_5MoEShardIndex, Qwen3_5MoEVisionConfig};
...
pub(super) fn validate_vision_tower_inventory(
...
) -> Result<ValidatedVisionTowerStorage, Qwen3_5MoEArtifactError> {
    ...
    let vision_config = vision_config.ok_or(Qwen3_5MoEArtifactError::MissingVisionConfig)?;

    let expected_vision_tensor_names = qwen3_5_moe_vision_tensor_profiles(vision_config)
        .into_iter()
    ...
    if let Some(unexpected_tensor_name) = actual_vision_tensor_names
        ...
        .next()
    {
        return Err(Qwen3_5MoEArtifactError::UnexpectedVisionTensor {
    ...
    if let Some(missing_tensor_name) = expected_vision_tensor_names
        ...
        .next()
    {
        return Err(Qwen3_5MoEArtifactError::MissingVisionTensor {
    ...
    if uses_separate_sidecar {
        ...
        {
            return Err(Qwen3_5MoEArtifactError::MixedVisionTensorStorage {
    ...
    for (tensor_name, shard_file_name) in vision_tensor_name_to_shard_file_name {
        ...
        {
            return Err(Qwen3_5MoEArtifactError::VisionTensorOutsideModelShards {
                tensor_name: tensor_name.clone(),
                shard_file_name: shard_file_name.clone(),
            });
        }
