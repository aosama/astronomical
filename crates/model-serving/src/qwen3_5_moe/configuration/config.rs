...
use super::config_document::{Qwen3_5MoEConfigDocument, Qwen3_5MoETextConfig};
use super::config_validation::{
    QWEN_CHAT_EOS_TOKEN_ID, Qwen3_5MoEConfigError, validate_exact_boolean, validate_exact_value,
};
...
/// Validated Qwen3.5-MoE text configuration used to derive execution shapes and behavior.
...
impl Qwen3_5MoEConfig {
    ...
    pub fn from_json_bytes(config_bytes: &[u8]) -> Result<Self, Qwen3_5MoEConfigError> {
        let config_document = serde_json::from_slice::<Qwen3_5MoEConfigDocument>(config_bytes)
            .map_err(Qwen3_5MoEConfigError::DeserializeConfig)?;
        let is_dense_model = config_document.model_type == EXPECTED_DENSE_MODEL_TYPE;
        let expected_architecture = if is_dense_model {
            EXPECTED_DENSE_ARCHITECTURE
        } else {
            EXPECTED_MOE_ARCHITECTURE
        };
        ...
        if !matches!(
            config_document.model_type.as_str(),
            EXPECTED_DENSE_MODEL_TYPE | EXPECTED_MOE_MODEL_TYPE
        ) {
            return Err(Qwen3_5MoEConfigError::InvalidConfigValueDynamic {
                description: format!(
        ...
        let activation_dtype = config_document
            ...
            .ok_or(Qwen3_5MoEConfigError::MissingActivationDtype)?;
        ...
        let resolved_eos_token_ids = match config_document.eos_token_id {
            ...
            None => config_document
                ...
                .clone()
                .ok_or(Qwen3_5MoEConfigError::MissingEosTokenId)?,
        };
        ...
        config_document.text_config.validate(is_dense_model)?;
        let (
            ...
            (Some(quantization), Some(quantization_config)) => {
                let quantized_module_profiles =
                    quantization.validate(&config_document.text_config)?;
                let mtp_quantized_module_profiles = quantization.mtp_quantized_module_profiles();
                quantization_config.validate(&config_document.text_config)?;
                if quantization != quantization_config {
                    return Err(Qwen3_5MoEConfigError::QuantizationCopiesDiffer);
                }
                (
                    quantized_module_profiles,
                    mtp_quantized_module_profiles,
    ...
    /// Returns whether this validated configuration executes the dense Qwen3.5 MLP path.
    #[must_use]
    pub const fn is_dense_model(&self) -> bool {
        self.is_dense_model
    }
