...
use super::{
    config_validation::{
        Qwen3_5MoEConfigError, deserialize_f32_bits, deserialize_optional_eos_token_ids,
...
pub(super) struct Qwen3_5MoEConfigDocument {
    ...
    pub(super) text_config: Qwen3_5MoETextConfig,
...
pub(super) struct Qwen3_5MoETextConfig {
    ...
    #[serde(default)]
    pub(super) rope_parameters: Option<Qwen3_5MoERopeParameters>,
    #[serde(
...
impl QuantizationConfigSource for Qwen3_5MoETextConfig {
    ...
    fn is_dense_model(&self) -> bool {
        self.num_experts == 0
    }
}

impl Qwen3_5MoETextConfig {
    pub(super) fn validate(&self, is_dense_model: bool) -> Result<(), Qwen3_5MoEConfigError> {
        let expected_text_model_type = if is_dense_model {
            EXPECTED_DENSE_TEXT_MODEL_TYPE
        } else {
            EXPECTED_MOE_TEXT_MODEL_TYPE
        };
        ...
        if self.hidden_size == 0
            ...
            || (is_dense_model && self.intermediate_size == 0)
        {
        ...
        if !is_dense_model && self.num_experts_per_tok > self.num_experts {
            return Err(Qwen3_5MoEConfigError::InvalidConfigValue {
        ...
        if self.layer_types.len() != self.num_hidden_layers as usize {
            return Err(Qwen3_5MoEConfigError::LayerTypeCountMismatch {
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub(super) struct Qwen3_5MoERopeParameters {
...
impl Qwen3_5MoERopeParameters {
    fn validate(&self) -> Result<(), Qwen3_5MoEConfigError> {
        ...
        if self.mrope_section != [11, 11, 10] {
            return Err(Qwen3_5MoEConfigError::MropeSectionMismatch {
