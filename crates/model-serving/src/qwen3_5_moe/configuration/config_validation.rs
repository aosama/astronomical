...
pub(in crate::qwen3_5_moe) fn validate_exact_value(
    field_name: &'static str,
    actual_value: &str,
    expected_value: &'static str,
) -> Result<(), Qwen3_5MoEConfigError> {
    if actual_value == expected_value {
        return Ok(());
    }
    Err(Qwen3_5MoEConfigError::UnexpectedStringValue {
        field_name,
        expected_value,
...
pub(super) fn validate_exact_boolean(
    ...
    Err(Qwen3_5MoEConfigError::UnexpectedBooleanValue {
        field_name,
        expected_value,
        actual_value,
    })
...
/// A config-decoding or implemented-execution-contract failure for Qwen3.5-MoE.
#[derive(Debug, Error)]
pub enum Qwen3_5MoEConfigError {
    /// The retained config bytes were not valid JSON for the expected document shape.
    #[error("failed to decode Qwen3.5-MoE config JSON")]
    DeserializeConfig(#[source] serde_json::Error),
    /// A required string field selected behavior this executor does not implement.
    #[error(
        "Qwen3.5-MoE config field '{field_name}' must be '{expected_value}', got '{actual_value}'"
    )]
    ...
    #[error("Qwen3.5-MoE config field '{field_name}' must be {expected_value}, got {actual_value}")]
    ...
    #[error(
        "Qwen3.5-MoE config declares {actual_layer_type_count} layer types, expected {expected_layer_type_count}"
    )]
    ...
    #[error("Qwen3.5-MoE config must specify `dtype` at the top level or inside `text_config`")]
    ...
    #[error(
        "Qwen3.5-MoE config must specify `eos_token_id` at the top level or inside `text_config`"
    )]
    MissingEosTokenId,
}
