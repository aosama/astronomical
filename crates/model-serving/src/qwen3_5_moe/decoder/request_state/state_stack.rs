...
/// One in-memory state entry per certified Qwen3.5-MoE decoder layer. Owns every
...
impl RequestDecoderStateStack {
    ...
    pub fn empty_from_decoder_cache_layout_with_full_attention_kv_state_growth_tokens(
        ...
        if full_attention_kv_state_growth_tokens <= 0 {
            return Err(MlxRuntimeError::RuntimeOperation {
                operation: "create Qwen3.5-MoE request decoder state",
    ...
    /// Creates empty decoder-layer state entries from the validated Qwen3.5-MoE
    /// config layer schedule. No MLX arrays are allocated until the first
    /// forward pass touches a layer.
    #[must_use]
    pub fn empty_from_config(qwen3_5_moe_config: &Qwen3_5MoEConfig) -> Self {
    ...
    pub fn empty_from_config_with_full_attention_kv_state_growth_tokens(
        ...
        let decoder_cache_layout =
            crate::qwen3_5_moe::decoder::cache_layout::qwen3_5_moe_decoder_cache_layout(
                qwen3_5_moe_config,
            )
    ...
    fn empty_from_config_with_validated_full_attention_kv_state_growth_tokens(
        ...
        let linear_convolution_kernel_dimension =
            qwen3_5_moe_config.linear_convolution_kernel_dimension() as i32;
        let linear_convolution_dimension = qwen3_5_moe_config.linear_convolution_state_dimension();
        let linear_value_head_count = qwen3_5_moe_config.linear_value_head_count() as i32;
        let linear_value_head_dimension = qwen3_5_moe_config.linear_value_head_dimension() as i32;
        let linear_key_head_dimension = qwen3_5_moe_config.linear_key_head_dimension() as i32;
        let layers = (0..qwen3_5_moe_config.layer_count() as usize)
    ...
    match decoder_cache_layer_layout {
        ...
        DecoderCacheLayerLayout::Composite { components } => {
            ...
            if convolution_tensor.qualified_role_name()
                != crate::qwen3_5_moe::decoder::cache_layout::QWEN_CONVOLUTION_TENSOR_ROLE
                || recurrent_tensor.qualified_role_name()
                    != crate::qwen3_5_moe::decoder::cache_layout::QWEN_RECURRENCE_TENSOR_ROLE
            {
                return Err(request_decoder_state_error(
