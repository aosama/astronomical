...
use super::super::model::memory_admission::{
    invalid_request_error, record_completed_adaptive_ram_growth, validate_context_memory_admission,
};
...
impl Qwen3_5MoEEngineState {
    pub(super) fn inject_input_tokens(
        ...
        let mut active_request = self.active_request.take().ok_or_else(|| {
            fatal_engine_error(
                "Qwen3.5-MoE model feedback injection requested without an active request",
            )
        })?;
        if active_request.request_id != request_id {
            self.active_request = Some(active_request);
            return Err(fatal_engine_error(
                "Qwen3.5-MoE model feedback injection request correlation mismatch",
            ));
        }
    ...
    fn inject_into_active_request(
        ...
        if !active_request.verified_mtp_generated_token_ids.is_empty() {
            ...
            active_request
                ...
                .map_err(super::qwen3_5_moe_runtime_error)?;
