...
use super::{
    Qwen3_5MoEEngineState, Qwen3_5MoEMtpRuntimeState, fatal_engine_error, qwen3_5_moe_runtime_error,
};
use crate::qwen3_5_moe::{
    Qwen3_5MoEImageProcessor, Qwen3_5MoEModel, Qwen3_5MoEMtpArtifactCapability,
...
impl Qwen3_5MoEEngineState {
    pub(super) fn load(&mut self) -> Result<EngineLoadResult, InferenceEngineError> {
        ...
        let mut qwen3_5_moe_mtp_artifact_capability = Qwen3_5MoEMtpArtifactCapability::TargetOnly;
        let model_loading_result: Result<_, InferenceEngineError> = (|| {
            let validated_artifact = self.validated_artifact.take().ok_or_else(|| {
                fatal_engine_error("validated Qwen3.5-MoE artifact is unavailable during MLX load")
            })?;
            let qwen3_5_moe_vision_config = validated_artifact.vision_config().cloned();
            qwen3_5_moe_mtp_artifact_capability =
            ...
            model_loading_performance_attribution
                ...
                .map_err(qwen3_5_moe_runtime_error)?;
            let mut model = Qwen3_5MoEModel::load_with_performance_attribution(
                runtime,
                validated_artifact,
            ...
            if !model.config().is_dense_model() {
            ...
            let persistent_visual_embedding_model_contract =
                qwen3_5_moe_vision_config.as_ref().map(|vision_config| {
                    let qwen3_5_moe_image_processor =
                        Qwen3_5MoEImageProcessor::from_vision_config(vision_config);
                ...
                self.persistent_prompt_cache_disk_store_config.clone()
            {
                ...
                match model_loading_performance_attribution.measure_operation(
                    ...
                    Ok(persistent_prompt_cache) => {
                        ...
                        {
                            tracing::warn!(
                                "Qwen3.5-MoE visual embedding cache could not be scanned; \
                        ...
                        tracing::info!(
                            ...
                            "opened Qwen3.5-MoE persistent prompt cache"
                        );
                    ...
                    Err(persistent_prompt_cache_error) => {
                        tracing::warn!(
                            "Qwen3.5-MoE persistent prompt cache could not be opened; \
        ...
        match model_loading_result {
            Ok((
                ...
                match self.mtp_runtime_state {
                    ...
                    Qwen3_5MoEMtpRuntimeState::TargetOnly => tracing::info!(
                    ...
                    Qwen3_5MoEMtpRuntimeState::Active => tracing::info!(
                        model_id = self.model_id.as_deref().unwrap_or("unknown"),
    ...
    fn engine_load_result_for_mtp_state(
        ...
        let mtp_runtime_state = match self.mtp_runtime_state {
            ...
            Qwen3_5MoEMtpRuntimeState::TargetOnly => {
                astronomical_ipc_protocol::MtpRuntimeState::TargetOnly
            }
            Qwen3_5MoEMtpRuntimeState::Active => astronomical_ipc_protocol::MtpRuntimeState::Active,
            Qwen3_5MoEMtpRuntimeState::Unavailable => {
        ...
        if self.mtp_runtime_state == Qwen3_5MoEMtpRuntimeState::Unavailable {
...
#[doc(hidden)]
#[must_use]
pub fn qwen3_5_moe_mtp_runtime_state_after_load(
    mtp_enabled: bool,
    mtp_artifact_capability: &Qwen3_5MoEMtpArtifactCapability,
    model_has_mtp_weights: bool,
) -> (Qwen3_5MoEMtpRuntimeState, Option<String>) {
    if !mtp_enabled {
        return (Qwen3_5MoEMtpRuntimeState::Disabled, None);
    }
    if model_has_mtp_weights {
        return (Qwen3_5MoEMtpRuntimeState::Active, None);
    }
    match mtp_artifact_capability {
        Qwen3_5MoEMtpArtifactCapability::TargetOnly => {
            (Qwen3_5MoEMtpRuntimeState::TargetOnly, None)
        }
        Qwen3_5MoEMtpArtifactCapability::MtpCapable { .. } => (
            Qwen3_5MoEMtpRuntimeState::Unavailable,
            Some(Qwen3_5MoEMtpUnavailableReason::NoCompatibleHead.to_string()),
        ),
        Qwen3_5MoEMtpArtifactCapability::InvalidMtp { reason } => (
            Qwen3_5MoEMtpRuntimeState::Unavailable,
