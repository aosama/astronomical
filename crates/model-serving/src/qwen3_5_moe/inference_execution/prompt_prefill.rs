...
use crate::{
    AdaptiveRamGrowthContext, InferenceEngineError, PerformanceCounter, Qwen3_5MoEExecutionError,
};

use super::super::model::memory_admission::AdaptiveRamGrowthMemoryAdmissionError;
use super::engine_request::{Qwen3_5MoEEngineRequest, Qwen3_5MoEPrefillRequestCheckpoint};
    ...
    pub(super) fn execute_prompt_prefill_chunck(
        ...
        if let Some(visual_embeddings) = active_request.visual_embeddings.as_ref() {
            let consumed_visual_embedding_count = match model
                ...
                Err(qwen3_5_moe_execution_error) => {
                    return Err(prefill_execution_error(
                        qwen3_5_moe_execution_error,
                        prefill_request_checkpoint,
                    ));
                }
            };
            ...
            if let Err(qwen3_5_moe_execution_error) = model
...
fn prefill_execution_error(
    qwen3_5_moe_execution_error: Qwen3_5MoEExecutionError,
    prefill_request_checkpoint: Qwen3_5MoEPrefillRequestCheckpoint,
) -> PromptPrefillChunckAttemptError {
    match qwen3_5_moe_execution_error {
        Qwen3_5MoEExecutionError::Runtime(MlxRuntimeError::ActiveMemoryLimitExceeded {
            active_memory_bytes,
        ...
        other_qwen3_5_moe_execution_error => {
            PromptPrefillChunckAttemptError::Engine(other_qwen3_5_moe_execution_error.into())
        }
    }
}
