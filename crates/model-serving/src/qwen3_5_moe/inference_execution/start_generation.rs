use crate::{
    ...
    PerformanceOperation, PersistentPromptCacheBlockKey, Qwen3_5MoEInferenceRequest,
    Qwen3_5MoESamplingStrategy,
};
...
use super::super::{
    Qwen3_5MoEMtpRequestState, RequestDecoderStateStack, plan_qwen3_5_moe_visual_embedding_suffix,
};
...
impl Qwen3_5MoEEngineState {
    pub(super) fn start_generation(
        &mut self,
        mut inference_request: Qwen3_5MoEInferenceRequest,
        ...
        if self.model.is_none() {
            return Err(fatal_engine_error("Qwen3.5-MoE engine is not loaded"));
        }
        ...
        let admitted_generation_start = (|| {
            ...
            let random_state = match sampling_strategy {
                Qwen3_5MoESamplingStrategy::Greedy => None,
                Qwen3_5MoESamplingStrategy::TopKTopP {
                    ...
                    let model = self.model.as_ref().ok_or_else(|| {
                        fatal_engine_error("Qwen3.5-MoE engine lost its loaded model")
                    })?;
            ...
            let mtp_is_eligible = self.mtp_enabled
                && self.mtp_runtime_state == super::Qwen3_5MoEMtpRuntimeState::Active
                && model_has_mtp_head
                && inference_request.sampling_strategy() == Qwen3_5MoESamplingStrategy::Greedy
                    ...
                    if let Some(persistent_prompt_cache) = persistent_prompt_cache.as_ref() {
                        match self.restore_persistent_prompt_cache_prefix(
                            ...
                            Err(persistent_prompt_cache_error) => {
                                tracing::warn!(
                                    "Qwen3.5-MoE persistent prompt-cache restore failed; \
                ...
                precomputed_visual_embeddings
            {
                ...
                let visual_embedding_suffix_plan = plan_qwen3_5_moe_visual_embedding_suffix(
                        &prompt_token_ids,
                        prefill_cursor,
                        &ordered_image_visual_embedding_row_counts,
                        image_pad_token_id,
            ...
            let mtp_request_state = if mtp_is_eligible {
                Some(
                    Qwen3_5MoEMtpRequestState::empty_with_growth_tokens(
                        self.full_attention_kv_state_growth_tokens,
                    )
                    .map_err(qwen3_5_moe_runtime_error)?,
                )
            ...
            self.active_request = Some(Qwen3_5MoEEngineRequest {
                request_decoder_state,
