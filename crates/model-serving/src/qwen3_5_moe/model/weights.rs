...
use super::{
    Qwen3_5MoEConfig, Qwen3_5MoEExecutionError, ValidatedQwen3_5MoEArtifact,
    decoder_layer_weights::{
        Qwen3_5DecoderLayerMlpWeights, Qwen3_5MoEAffineWeights, Qwen3_5MoEAttentionWeights,
        Qwen3_5MoEDecoderLayerWeights, Qwen3_5MoEFullAttentionWeights,
        Qwen3_5MoELinearAttentionWeights, Qwen3_5MoEMixtureOfExpertsWeights, RouterGateWeights,
    },
    qwen3_5_moe_resident_language_tensor_profiles,
};

/// Strict typed ownership for the five executable Qwen3.5-MoE language shards.
#[derive(Debug)]
pub struct Qwen3_5MoEWeights {
    pub(super) embedding_weights: Qwen3_5MoEAffineWeights,
    pub(super) decoder_layer_weights: Vec<Qwen3_5MoEDecoderLayerWeights>,
    pub(super) final_normalization_weight: MlxArray,
    pub(super) language_model_head_weights: Qwen3_5MoEAffineWeights,
    model_shards: Vec<MlxSafetensors>,
    tensor_count: usize,
    total_payload_bytes: u64,
}

impl Qwen3_5MoEWeights {
    ...
    pub fn load_with_performance_attribution(
        ...
        performance_attribution.measure_operation(
            ...
            |_performance_attribution| {
                Self::bind_from_model_shards(&qwen3_5_moe_config, &shard_index, model_shards)
            },
        )
    }
    ...
    pub fn bind_from_model_shards(
        ...
        let language_tensor_profiles =
            qwen3_5_moe_resident_language_tensor_profiles(qwen3_5_moe_config);
        ...
        for tensor_profile in &language_tensor_profiles {
            ...
            actual_payload_bytes = actual_payload_bytes
                ...
                .ok_or(Qwen3_5MoEExecutionError::InvalidTensor {
                    tensor_name: tensor_profile.name.clone(),
        ...
        let decoder_layer_count = qwen3_5_moe_config.layer_count() as usize;
...
fn take_decoder_layer_weights(
...
) -> Result<Qwen3_5MoEDecoderLayerWeights, Qwen3_5MoEExecutionError> {
    ...
    let attention_weights =
        if qwen3_5_moe_config.decoder_layer_is_full_attention(decoder_layer_index) {
            Qwen3_5MoEAttentionWeights::Full(take_full_attention_weights(
                bound_tensors,
                qwen3_5_moe_config,
                &decoder_layer_prefix,
            )?)
        } else {
            Qwen3_5MoEAttentionWeights::Linear(take_linear_attention_weights(
                bound_tensors,
                qwen3_5_moe_config,
                &decoder_layer_prefix,
            )?)
    ...
    let mlp_weights = if qwen3_5_moe_config.is_dense_model() {
    ...
    Ok(Qwen3_5MoEDecoderLayerWeights {
...
) -> Result<Qwen3_5MoELinearAttentionWeights, Qwen3_5MoEExecutionError> {
    let linear_attention_prefix = format!("{decoder_layer_prefix}.linear_attn");
    Ok(Qwen3_5MoELinearAttentionWeights {
...
pub(super) fn take_full_attention_weights(
    ...
    qwen3_5_moe_config: &Qwen3_5MoEConfig,
    decoder_layer_prefix: &str,
) -> Result<Qwen3_5MoEFullAttentionWeights, Qwen3_5MoEExecutionError> {
    let full_attention_prefix = format!("{decoder_layer_prefix}.self_attn");
    Ok(Qwen3_5MoEFullAttentionWeights {
...
pub(super) fn take_mixture_of_experts_weights(
    bound_tensors: &mut HashMap<String, MlxArray>,
    qwen3_5_moe_config: &Qwen3_5MoEConfig,
    decoder_layer_prefix: &str,
) -> Result<Qwen3_5MoEMixtureOfExpertsWeights, Qwen3_5MoEExecutionError> {
    let mixture_of_experts_prefix = format!("{decoder_layer_prefix}.mlp");
    let gate_module_name = format!("{mixture_of_experts_prefix}.gate");
    // The MoE router gate can be either quantized (with .weight/.scales/.biases)
    // or unquantized (bfloat16, with only .weight). When .scales is absent from
    // the bound tensors, the gate is bfloat16 regardless of what the config
    // quantization map says. This handles models like oQ6e where the config
    // default bits=6 would imply quantization, but the actual weights are bfloat16.
    let gate_scales_name = format!("{gate_module_name}.scales");
    let router_projection = if bound_tensors.contains_key(&gate_scales_name) {
        RouterGateWeights::Affine(take_quantized_affine_weights(
            bound_tensors,
            qwen3_5_moe_config,
            &gate_module_name,
        )?)
    } else {
        // Unquantized gate: stored as plain bfloat16 weight, no scales/biases
        RouterGateWeights::Unquantized(take_tensor(
            bound_tensors,
            format!("{gate_module_name}.weight"),
        )?)
    };
    let take_projection = |bound_tensors: &mut HashMap<String, MlxArray>, suffix: &str| {
    ...
    Ok(Qwen3_5MoEMixtureOfExpertsWeights {
        router_projection,
        shared_expert_gate_projection: take_projection(bound_tensors, "shared_expert.gate_proj")?,
        shared_expert_up_projection: take_projection(bound_tensors, "shared_expert.up_proj")?,
        shared_expert_down_projection: take_projection(bound_tensors, "shared_expert.down_proj")?,
        shared_expert_output_gate_projection: take_projection(bound_tensors, "shared_expert_gate")?,
    })
}

pub(super) fn take_quantized_affine_weights(
    bound_tensors: &mut HashMap<String, MlxArray>,
    qwen3_5_moe_config: &Qwen3_5MoEConfig,
    module_name: &str,
) -> Result<Qwen3_5MoEAffineWeights, Qwen3_5MoEExecutionError> {
    ...
    if quantization_profile.is_unquantized() {
        return Ok(Qwen3_5MoEAffineWeights::NativeBfloat16 {
...
pub(super) fn take_tensor(
    ...
    bound_tensors
        .remove(&tensor_name)
        .ok_or(Qwen3_5MoEExecutionError::MissingTensor { tensor_name })
}
