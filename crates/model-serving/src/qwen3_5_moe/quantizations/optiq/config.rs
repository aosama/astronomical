...
use crate::qwen3_5_moe::configuration::config_validation::{
    Qwen3_5MoEConfigError, validate_exact_value,
};
...
///
/// Each quantized module in a Qwen3.5-MoE model can have its own bit width
...
/// The two identical quantization documents embedded in the Qwen3.5-MoE config.
///
...
impl OptiQQuantizationConfig {
    ...
    pub fn validate(
    ...
    ) -> Result<BTreeMap<String, OptiQQuantizationProfile>, Qwen3_5MoEConfigError> {
        ...
        for (module_name, quantization_override) in &self.module_overrides {
            if !is_mlx_affine_quantization_bit_width_supported(quantization_override.bits) {
                return Err(Qwen3_5MoEConfigError::UnsupportedQuantizationOverrideBits {
        ...
        let all_module_names = expected_quantized_module_names(config_source);
...
fn expected_quantized_module_names(config_source: &impl QuantizationConfigSource) -> Vec<String> {
    ...
    for decoder_layer_index in 0..layer_count {
        ...
        let mlp_module_suffixes: &[&str] = if config_source.is_dense_model() {
