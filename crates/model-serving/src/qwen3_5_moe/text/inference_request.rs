...
/// Token-level request prepared for Qwen3.5-MoE inference.
#[derive(Clone, Debug)]
pub struct Qwen3_5MoEInferenceRequest {
    ...
    sampling_strategy: Qwen3_5MoESamplingStrategy,
    thinking_budget: Option<u16>,
    performance_attribution: PerformanceAttribution,
}

impl PreparedInferenceRequest for Qwen3_5MoEInferenceRequest {
    fn prompt_token_count(&self) -> usize {
        self.input_token_ids.len()
    }
}

impl Qwen3_5MoEInferenceRequest {
    /// Creates a token-level request accepted by Qwen3.5-MoE inference.
    pub fn new(request_id: RequestId, input_token_ids: Vec<u32>, max_output_tokens: u16) -> Self {
        Self {
            ...
            max_output_tokens,
            request_id,
            sampling_strategy: Qwen3_5MoESamplingStrategy::Greedy,
            thinking_budget: None,
            performance_attribution: PerformanceAttribution::disabled(),
        }
    }

    /// Creates a token-level request using Qwen3.5-MoE's fixed top-k sampling family.
    pub fn new_sampling(
        request_id: RequestId,
        input_token_ids: Vec<u32>,
        max_output_tokens: u16,
    ...
    #[must_use]
    pub fn processed_visual_images(&self) -> &[Qwen3_5MoEProcessedImage] {
    ...
    /// Returns the selected Qwen3.5-MoE sampling strategy.
    #[must_use]
    pub const fn sampling_strategy(&self) -> Qwen3_5MoESamplingStrategy {
        self.sampling_strategy
    }
}

/// Sampling behavior selected for Qwen3.5-MoE token generation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Qwen3_5MoESamplingStrategy {
