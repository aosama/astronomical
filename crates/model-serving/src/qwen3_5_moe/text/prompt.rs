...
/// Renders the pinned Qwen3.5-MoE text chat template without executing model-provided Jinja.
#[derive(Debug)]
pub struct Qwen3_5MoEPromptRenderer;

impl Qwen3_5MoEPromptRenderer {
    /// Renders bounded typed chat history and the exact generation prefix for Qwen3.5-MoE.
    ///
    ...
    pub fn render(
    ...
    ) -> Result<String, Qwen3_5MoEPromptError> {
        if messages.is_empty() {
            return Err(Qwen3_5MoEPromptError::MissingMessages);
        }
        ...
        for (message_index, message) in messages.iter().enumerate() {
            match message {
                ...
                ChatMessage::System { .. } => {
                    return Err(Qwen3_5MoEPromptError::SystemMessageMustBeFirst);
                }
...
fn render_assistant_tool_call(
    ...
    let argument_values = serde_json::from_str::<Value>(&tool_call.function.arguments_json)
        .map_err(|source| Qwen3_5MoEPromptError::InvalidToolArguments {
            function_name: tool_call.function.name.clone(),
            source,
        })?;
    let Value::Object(argument_values) = argument_values else {
        return Err(Qwen3_5MoEPromptError::ToolArgumentsMustBeObject {
            function_name: tool_call.function.name.clone(),
        });
    };
...
fn render_tool_definition(tool: &ChatToolDefinition) -> Result<String, Qwen3_5MoEPromptError> {
    let parameters = serde_json::from_str::<Value>(&tool.parameters_json).map_err(|source| {
        Qwen3_5MoEPromptError::InvalidToolParameters {
            function_name: tool.name.clone(),
            source,
        }
    })?;
    let function_name_json = serde_json::to_string(&tool.name).map_err(|source| {
        Qwen3_5MoEPromptError::SerializeToolDefinition {
            function_name: tool.name.clone(),
            source,
        }
    })?;
    ...
    let description_json = tool
        ...
        .transpose()
        .map_err(|source| Qwen3_5MoEPromptError::SerializeToolDefinition {
            function_name: tool.name.clone(),
            source,
        })?;
...
fn render_parameter_value(parameter_value: &Value) -> Result<String, Qwen3_5MoEPromptError> {
    match parameter_value {
        Value::String(string_value) => Ok(string_value.clone()),
        Value::Null | Value::Bool(_) | Value::Number(_) | Value::Array(_) | Value::Object(_) => {
            serde_json::to_string(parameter_value)
                .map_err(Qwen3_5MoEPromptError::SerializeToolArgument)
        }
    }
}
...
/// A typed error while reproducing the reviewed Qwen3.5-MoE text template.
#[derive(Debug, Error)]
pub enum Qwen3_5MoEPromptError {
    /// At least one user query or tool-response sequence is required by the template.
    #[error("Qwen3.5-MoE prompt rendering requires at least one message")]
    MissingMessages,
    /// Only the first chat message may use the system role.
    #[error("Qwen3.5-MoE system messages must be the first conversation message")]
