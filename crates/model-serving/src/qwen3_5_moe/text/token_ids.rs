...
/// Token IDs and their corresponding string content from the Qwen3.5-MoE vocabulary.
///
...
/// because the Qwen3.5-MoE family shares the same special-token strings.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Qwen3_5MoETokenIds {
...
/// Discovers the special token IDs from a loaded Qwen3.5-MoE-family tokenizer.
///
/// The Qwen3.5-MoE family shares the same special token strings, but the numeric IDs
...
pub fn discover_token_ids(
    tokenizer: &Tokenizer,
) -> Result<Qwen3_5MoETokenIds, Qwen3_5MoETokenDiscoveryError> {
...
) -> Result<u32, Qwen3_5MoETokenDiscoveryError> {
    let token_id = tokenizer
        .token_to_id(token_content)
        .ok_or(Qwen3_5MoETokenDiscoveryError::MissingSpecialToken { token_content })?;
    let round_trip_content = tokenizer
        .id_to_token(token_id)
        .ok_or(Qwen3_5MoETokenDiscoveryError::MissingSpecialToken { token_content })?;
    if round_trip_content != token_content {
        return Err(
            Qwen3_5MoETokenDiscoveryError::SpecialTokenIdentityMismatch {
                token_content,
...
pub enum Qwen3_5MoETokenDiscoveryError {
