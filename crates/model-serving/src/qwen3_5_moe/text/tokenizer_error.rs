...
use super::{
    Qwen3_5MoEImageProcessingError, Qwen3_5MoEPromptError, token_ids::Qwen3_5MoETokenDiscoveryError,
};

/// A bounded failure while loading or using the pinned tokenizer.
#[derive(Debug, Error)]
pub enum Qwen3_5MoETokenizerError {
    ...
    RenderPrompt(#[source] Qwen3_5MoEPromptError),
    #[error("failed to process chat image input through the vision pipeline: {0}")]
    ImageProcessing(#[source] Qwen3_5MoEImageProcessingError),
    ...
    DiscoverTokenIds {
        #[source]
        source: Qwen3_5MoETokenDiscoveryError,
    },
}
