//! Host-side geometry plan for one Qwen3.5-MoE vision forward pass.
//!
...
use super::{Qwen3_5MoEImageGrid, Qwen3_5MoEVisionConfig};

const BILINEAR_CORNER_COUNT: usize = 4;

/// CPU-computed, image-dependent coordinates consumed by the Qwen3.5-MoE vision tower.
///
...
pub struct Qwen3_5MoEVisionInputPlan {
    patch_count: usize,
...
impl Qwen3_5MoEVisionInputPlan {
    /// Builds the data-dependent position plan used by the pinned vision architecture.
    pub fn new(
        image_grids: &[Qwen3_5MoEImageGrid],
        vision_config: &Qwen3_5MoEVisionConfig,
    ) -> Result<Self, Qwen3_5MoEVisionInputPlanError> {
        if image_grids.is_empty() {
            return Err(Qwen3_5MoEVisionInputPlanError::MissingImageGrids);
        }
        ...
        let spatial_merge_area = usize::try_from(
            spatial_merge_size
                ...
                .ok_or(Qwen3_5MoEVisionInputPlanError::DimensionOverflow)?,
        )
        .map_err(|_conversion_error| Qwen3_5MoEVisionInputPlanError::DimensionOverflow)?;
...
/// Invalid dynamic image geometry for the pinned Qwen3.5-MoE vision architecture.
#[derive(Debug, Error, Clone, Copy, Eq, PartialEq)]
pub enum Qwen3_5MoEVisionInputPlanError {
    #[error("at least one image grid is required for Qwen3.5-MoE vision execution")]
    MissingImageGrids,
...
) -> Result<(), Qwen3_5MoEVisionInputPlanError> {
...
fn validate_image_grid(
    ...
    {
        return Err(Qwen3_5MoEVisionInputPlanError::NonPositiveImageGridDimension);
    }
    if !image_grid
        .height_patch_count
        ...
        || !image_grid
            ...
            .is_multiple_of(spatial_merge_size)
    {
        return Err(Qwen3_5MoEVisionInputPlanError::ImageGridNotDivisibleBySpatialMergeSize);
    }
    Ok(())
}

fn exact_square_side(position_embedding_count: u32) -> Result<u32, Qwen3_5MoEVisionInputPlanError> {
    ...
    if candidate_grid_side.checked_mul(candidate_grid_side) != Some(position_embedding_count) {
        return Err(Qwen3_5MoEVisionInputPlanError::PositionEmbeddingCountNotSquare);
    }
