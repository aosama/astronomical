...
use super::super::inference_execution::{
    Qwen3_5MoEEngineState, fatal_engine_error, qwen3_5_moe_runtime_error,
};
#[cfg(feature = "direct-mlx")]
use super::Qwen3_5MoEProcessedImage;
#[cfg(feature = "direct-mlx")]
use crate::{
...
pub struct Qwen3_5MoEVisualEmbeddingRequiredImage {
    image_index: usize,
...
impl Qwen3_5MoEVisualEmbeddingRequiredImage {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Qwen3_5MoEVisualEmbeddingSuffixPlan {
    restored_visual_embedding_row_count: usize,
    remaining_visual_embedding_row_count: usize,
    required_images: Vec<Qwen3_5MoEVisualEmbeddingRequiredImage>,
}

impl Qwen3_5MoEVisualEmbeddingSuffixPlan {
    ...
    #[must_use]
    pub fn required_images(&self) -> &[Qwen3_5MoEVisualEmbeddingRequiredImage] {
        &self.required_images
    }
}

/// Plans visual embedding rows needed after restoring a prompt prefix.
pub fn plan_qwen3_5_moe_visual_embedding_suffix(
    prompt_token_ids: &[u32],
    restored_token_count: usize,
    ordered_image_visual_embedding_row_counts: &[usize],
    image_pad_token_id: u32,
) -> Result<Qwen3_5MoEVisualEmbeddingSuffixPlan, Qwen3_5MoEVisualEmbeddingSuffixPlanError> {
    if restored_token_count > prompt_token_ids.len() {
        return Err(
            Qwen3_5MoEVisualEmbeddingSuffixPlanError::RestoredTokenCountExceedsPrompt {
    ...
    {
        return Err(Qwen3_5MoEVisualEmbeddingSuffixPlanError::ZeroImageRows { image_index });
    }
    ...
    let total_visual_embedding_row_count = ordered_image_visual_embedding_row_counts
        ...
        .ok_or(Qwen3_5MoEVisualEmbeddingSuffixPlanError::ImageRowCountOverflow)?;
    if prompt_image_pad_token_count != total_visual_embedding_row_count {
        return Err(
            Qwen3_5MoEVisualEmbeddingSuffixPlanError::ImagePadCountMismatch {
        ...
        .enumerate()
    {
        ...
        required_images.push(Qwen3_5MoEVisualEmbeddingRequiredImage::new(
            image_index,
            image_visual_embedding_row_count,
            suffix_start_row,
            suffix_row_count,
    ...
    Ok(Qwen3_5MoEVisualEmbeddingSuffixPlan {
...
pub enum Qwen3_5MoEVisualEmbeddingSuffixPlanError {
    #[error(
...
impl Qwen3_5MoEEngineState {
    pub(in crate::qwen3_5_moe) fn resolve_visual_embeddings_for_processed_images(
        &mut self,
        request_id: RequestId,
        processed_visual_images: &[Qwen3_5MoEProcessedImage],
        visual_embedding_suffix_plan: &Qwen3_5MoEVisualEmbeddingSuffixPlan,
        ...
        let persistent_visual_embedding_model_contract = self
            ...
            .ok_or_else(|| {
                fatal_engine_error(
                    "Qwen3.5-MoE persistent visual embedding model contract is not loaded",
                )
            })?;
        ...
        if !missing_processed_visual_images.is_empty() {
            let vision_model = model.vision_model().ok_or_else(|| {
                fatal_engine_error("Qwen3.5-MoE image request lost the loaded vision model")
            })?;
            ...
            if !visual_embedding_array_has_expected_layout(
                ...
                return Err(fatal_engine_error(
                    "computed visual embeddings do not match expected Qwen3.5-MoE text width",
                ));
            }
