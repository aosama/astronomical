...
/// One independently timed fixed-depth or target-only qualification cell.
///
...
pub(crate) fn validate_mtp_depth_release_gate(
    ...
    for measurement in all_measurements {
        if measurement.output_token_count != expected_output_token_count {
            return Err("a qualification cell did not generate the required token count");
        }
        if measurement.maximum_active_mlx_memory_bytes > measurement.mlx_memory_ceiling_bytes {
            return Err("a qualification cell exceeded its active MLX memory ceiling");
        }
        ...
        if measurement.maximum_peak_mlx_memory_bytes > allowed_peak_bytes {
            return Err("a qualification cell exceeded its approved MLX peak allowance");
        }
        if measurement.operational_fallback_count != 0 {
            return Err("a qualification cell used operational target-only fallback");
        }
    }
    ...
    for (expected_depth, measurement) in [(1_u8, depth_one), (2, depth_two), (3, depth_three)] {
        if measurement.draft_depth != Some(expected_depth) {
            return Err("an MTP qualification cell reported the wrong fixed depth");
        }
        ...
        {
            return Err("an MTP qualification cell did not exercise its requested depth");
        }
