use astronomical_model_serving::{
    qwen3_5_moe_gated_delta_kernel, qwen3_5_moe_gated_delta_sequence, qwen3_5_moe_gated_delta_step,
};
...
async fn should_apply_one_ops_based_gated_delta_recurrent_step() {
    ...
    let (output_values, next_recurrent_state) = qwen3_5_moe_gated_delta_step(
        &runtime,
        &queries,
        &keys,
        &values,
...
async fn should_match_ops_loop_when_applying_fused_gated_delta_sequence() {
    ...
    let gated_delta_kernel =
        qwen3_5_moe_gated_delta_kernel().expect("the fused gated-delta kernel should construct");
    let (fused_outputs, fused_recurrent_state) = qwen3_5_moe_gated_delta_sequence(
        &runtime,
        &gated_delta_kernel,
        &queries,
        &keys,
...
fn apply_ops_loop(
    ...
    for token_index in 0..token_count {
        ...
        let (token_output, next_recurrent_state) = qwen3_5_moe_gated_delta_step(
            runtime,
            &token_queries,
            &token_keys,
            &token_values,
