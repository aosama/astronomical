...
use astronomical_runtime_integration::{MlxCompiledSwiGlu, MlxDtype, MlxMemoryLimits, MlxRuntime};
...
#[tokio::test]
async fn should_fuse_four_bit_affine_gate_up_swiglu_for_one_decode_token() {
    let _direct_mlx_guard = crate::common::direct_mlx_test_guard().await;
    let runtime = MlxRuntime::initialize(
        MlxMemoryLimits::new(
            DIRECT_MLX_TEST_ACTIVE_MEMORY_LIMIT_BYTES,
        ...
        .expect("the fused SwiGLU test memory limits should be valid"),
    )
    .expect("the direct MLX runtime should initialize");
    let hidden_states = runtime
        .array_from_f32(
            &(1..=64).map(|index| index as f32).collect::<Vec<_>>(),
            &[1, 1, 64],
        )
        .and_then(|float_hidden_states| runtime.astype(&float_hidden_states, MlxDtype::BFloat16))
        .expect("the decode hidden states should use bfloat16");
    let gate_packed_weights = runtime
        .array_from_u32(&[0x1111_1111; 16], &[2, 8])
        .expect("the packed gate weights should be valid");
    let up_packed_weights = runtime
        .array_from_u32(&[0x2222_2222; 16], &[2, 8])
        .expect("the packed up weights should be valid");
    let gate_scales = runtime
        .array_from_f32(&[1.0; 2], &[2, 1])
        .and_then(|float_scales| runtime.astype(&float_scales, MlxDtype::BFloat16))
        .expect("the gate scales should use bfloat16");
    let up_scales = runtime
        .array_from_f32(&[1.0; 2], &[2, 1])
        .and_then(|float_scales| runtime.astype(&float_scales, MlxDtype::BFloat16))
        .expect("the up scales should use bfloat16");
    let gate_biases = runtime
        .array_from_f32(&[0.0; 2], &[2, 1])
        .and_then(|float_biases| runtime.astype(&float_biases, MlxDtype::BFloat16))
        .expect("the gate biases should use bfloat16");
    let up_biases = runtime
        .array_from_f32(&[0.0; 2], &[2, 1])
        .and_then(|float_biases| runtime.astype(&float_biases, MlxDtype::BFloat16))
        .expect("the up biases should use bfloat16");
    let compiled_swiglu = MlxCompiledSwiGlu::new().expect("the SwiGLU reference should compile");
    let reference_gate = runtime
        ...
        .expect("the reference gate projection should build");
    let reference_up = runtime
        .quantized_matmul_affine(
            &hidden_states,
            &up_packed_weights,
            &up_scales,
        ...
        .expect("the reference up projection should build");
    let reference_activated = runtime
        .apply_compiled_swiglu(&compiled_swiglu, &reference_gate, &reference_up)
        .expect("the reference SwiGLU should build");

    let fused_kernel =
        astronomical_model_serving::qwen3_5_moe_fused_four_bit_affine_gate_up_swiglu_kernel()
            .expect("the fused gate-up kernel should initialize");
    let fused_activated =
        astronomical_model_serving::qwen3_5_moe_fused_four_bit_affine_gate_up_swiglu(
            &runtime,
            &fused_kernel,
            &hidden_states,
            &gate_packed_weights,
        ...
        .expect("the fused gate-up SwiGLU should build");

    assert_eq!(fused_activated.shape(), vec![1, 1, 2]);
    assert_f32_close(
        &runtime
            .astype(&fused_activated, MlxDtype::Float32)
            .expect("the fused result should convert to float32")
            .to_vec_f32()
            .expect("the fused result should evaluate"),
        &runtime
            .astype(&reference_activated, MlxDtype::Float32)
            .expect("the reference result should convert to float32")
            .to_vec_f32()
            .expect("the reference result should evaluate"),
    );
}
