//! Checks tokenizer behavior from a validated Ornith artifact without
...
fn should_load_the_large_sparse_moe_tokenizer_and_expose_control_tokens() {
...
#[test]
#[ignore = "requires model_directories to discover the Ornith 1.5 acceptance artifact"]
fn should_prepare_a_validated_structured_chat_command_for_prefill() {
    let tokenizer = load_tokenizer();
...
fn should_decode_generated_tokens_into_separate_reasoning_and_text_events() {
    ...
    // parser acceptance valid when a packaging variant changes token ids.
