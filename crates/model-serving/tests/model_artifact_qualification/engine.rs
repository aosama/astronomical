...
use astronomical_model_serving::{
    ...
    Qwen3_5MoEPrefillChunckSizer, Qwen3_5MoETokenizer,
};
    ...
    qwen3_5_moe_engine
        .start_generation(
            Qwen3_5MoEInferenceRequest::new(
                request_id,
                super::SAY_HI_PROMPT_TOKEN_IDS.to_vec(),
...
async fn should_generate_the_certified_sampled_continuation_through_the_engine_trait() {
    ...
    qwen3_5_moe_engine
        .start_generation(
            Qwen3_5MoEInferenceRequest::new_sampling(
                request_id,
...
async fn should_keep_the_complete_model_resident_when_idle_memory_is_sufficient() {
    timeout(Duration::from_secs(120), async {
        ...
        let think_end_token_id = Qwen3_5MoETokenizer::from_validated_artifact(&validated_artifact)
            .expect("the tokenizer should expose validated control tokens")
            .think_end_token_id();
        let mlx_memory_limits = crate::common::sample_model_artifact_qualification_mlx_memory_limits().await;
        let mut qwen3_5_moe_engine = Qwen3_5MoEEngine::new_with_prefill_chunck_sizer_and_performance_attribution(
            validated_artifact,
            mlx_memory_limits.active_memory_limit_bytes(),
            mlx_memory_limits.allocator_cache_memory_limit_bytes(),
            None,
        ...
        let generation_finalization = qwen3_5_moe_engine
...
async fn should_use_the_raised_live_memory_limit_for_adaptive_expert_eviction() {
    timeout(Duration::from_secs(120), async {
        ...
        let retained_expert_payload_bytes_after_request_admission = qwen3_5_moe_engine
        ...
        let retained_expert_payload_bytes_after_adaptive_prefill = qwen3_5_moe_engine
            .collect_mlx_memory_telemetry()
            .await
