...
use astronomical_model_serving::{
    ExpertWeightMemoryCache, Qwen3_5MoEArtifactValidator, Qwen3_5MoEModel, RequestDecoderStateStack,
};
...
async fn run_direct_paged_decode_cache_reuse_proof() {
    ...
    run_one_say_hi_paged_decode(&qwen3_5_moe_model, &config, "first")
        .expect("first direct paged decode should complete");
    let after_first_decode_statistics = qwen3_5_moe_model.expert_weight_memory_cache_statistics();
    eprintln!(
    ...
    run_one_say_hi_paged_decode(&qwen3_5_moe_model, &config, "second")
        .expect("second direct paged decode should complete");
    let after_second_decode_statistics = qwen3_5_moe_model.expert_weight_memory_cache_statistics();
    eprintln!(
...
fn run_one_say_hi_paged_decode(
    ...
    config: &astronomical_model_serving::Qwen3_5MoEConfig,
    decode_label: &str,
) -> Result<(), astronomical_model_serving::Qwen3_5MoEExecutionError> {
    ...
    let final_position_logits = qwen3_5_moe_model.forward_chunk(
        &SAY_HI_PROMPT_TOKEN_IDS[SAY_HI_PROMPT_TOKEN_IDS.len() - 1..],
        (SAY_HI_PROMPT_TOKEN_IDS.len() - 1) as u32,
        &mut request_decoder_state,
    )?;
    let first_token_id = qwen3_5_moe_model.greedy_token_id(&final_position_logits)?;
    eprintln!(
