...
use astronomical_model_serving::{
    ExpertWeightMemoryCacheStatistics, Qwen3_5MoEArtifactValidator, Qwen3_5MoEConfig,
    Qwen3_5MoEModel, RequestDecoderStateStack,
};
...
use super::SAY_HI_PROMPT_TOKEN_IDS;
...
async fn run_30_token_paged_decode_cache_probe() {
    ...
    let (qwen3_5_moe_model, config) =
        load_paged_qwen3_5_moe_model_for_decode_probe("[paged-30]").await;
...
async fn run_30_token_warm_cache_decode_probe() {
    ...
    let test_started_at = Instant::now();
    let (qwen3_5_moe_model, config) =
        load_paged_qwen3_5_moe_model_for_decode_probe("[paged-30-warm]").await;
...
pub(crate) async fn load_paged_qwen3_5_moe_model_for_decode_probe(
    log_prefix: &str,
) -> (Qwen3_5MoEModel, Qwen3_5MoEConfig) {
...
fn run_say_hi_30_token_paged_decode_pass(
    ...
    let initial_cache_statistics = qwen3_5_moe_model.expert_weight_memory_cache_statistics();
    ...
    for decode_step_index in 0..GENERATED_TOKEN_COUNT {
        ...
        let before_step_cache_statistics =
            qwen3_5_moe_model.expert_weight_memory_cache_statistics();
        let step_started_at = Instant::now();
        let logits = qwen3_5_moe_model
            .forward_chunk(
                &[current_token_id],
                position_tokens,
                &mut request_decoder_state,
        ...
        current_token_id = qwen3_5_moe_model
        ...
        let after_step_cache_statistics = qwen3_5_moe_model.expert_weight_memory_cache_statistics();
        let mlx_memory_snapshot = qwen3_5_moe_model
            .runtime()
            .memory_snapshot()
    ...
    let final_cache_statistics = qwen3_5_moe_model.expert_weight_memory_cache_statistics();
