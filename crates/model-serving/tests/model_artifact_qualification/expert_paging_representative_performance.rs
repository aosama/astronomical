...
use astronomical_model_serving::{
    Qwen3_5MoEArtifactValidator, Qwen3_5MoEConfig, Qwen3_5MoEModel, RequestDecoderStateStack,
};
use astronomical_runtime_integration::{MlxMemoryLimits, MlxRuntime};

use super::expert_paging_decode::{
    bytes_to_gib, load_paged_qwen3_5_moe_model_for_decode_probe,
...
async fn run_representative_performance_probe() {
    ...
    let (qwen3_5_moe_model, config) =
        load_paged_qwen3_5_moe_model_for_decode_probe("[paged-representative]").await;
...
async fn run_oq4e_representative_performance_probe() {
    ...
    let qwen3_5_moe_model =
        Qwen3_5MoEModel::load(runtime, validated_artifact, &model_directory, false).expect(
            "the oQ4e model should load with demand paging and a configured 10 GB MLX limit",
        );
    assert_eq!(
        qwen3_5_moe_model.expert_storage_format(),
...
pub(crate) fn run_representative_performance_probe_for_loaded_model(
    ...
    for output_token_index in 0..REPRESENTATIVE_OUTPUT_TOKEN_COUNT {
            ...
            || completed_output_token_count == REPRESENTATIVE_OUTPUT_TOKEN_COUNT
        {
            ...
            let cache_statistics = qwen3_5_moe_model.expert_weight_memory_cache_statistics();
    ...
    let final_mlx_memory_snapshot = qwen3_5_moe_model
        .runtime()
        .memory_snapshot()
