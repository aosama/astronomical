...
async fn run_expert_route_reuse_probe(
    ...
    let (mut qwen3_5_moe_engine, end_of_sequence_token_ids) = create_attributed_engine(
        &model_directory,
        &performance_attribution_log_path,
        mlx_memory_limits,
        FIXED_PREFILL_CHUNCK_TOKENS,
    );
    load_engine_with_progress(&mut qwen3_5_moe_engine, progress_log_label).await;
