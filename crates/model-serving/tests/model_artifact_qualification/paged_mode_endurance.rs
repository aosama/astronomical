...
async fn run_automatic_residency_endurance_regression() {
    ...
    let (mut qwen3_5_moe_engine, _temporary_log_directory, performance_attribution_log_path) =
    ...
    load_engine_with_progress(&mut qwen3_5_moe_engine).await;
...
) -> (Qwen3_5MoEEngine, tempfile::TempDir, PathBuf) {
...
async fn load_engine_with_progress(qwen3_5_moe_engine: &mut Qwen3_5MoEEngine) {
...
async fn run_generation_until_first_token(
    ...
    loop {
        ...
        progress_interval.set_missed_tick_behavior(MissedTickBehavior::Delay);
        progress_interval.tick().await;
        let generation_advance_future = qwen3_5_moe_engine.decode_next_token(request_id);
