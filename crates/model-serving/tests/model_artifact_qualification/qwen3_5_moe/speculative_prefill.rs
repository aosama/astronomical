...
async fn should_measure_representative_speculative_prefill_against_a_target_only_control() {
    tokio::time::timeout(Duration::from_secs(115), async {
        ...
        assert_eq!(
            ...
            0,
            "disabled persistent caching must report no drafter SSD reuse",
        );
        assert_eq!(
            speculative_prefill_measurement.restored_target_persistent_prompt_cache_token_count,
            0,
            "disabled persistent caching must report no exact target SSD reuse",
        );
        assert_eq!(
            speculative_prefill_measurement
                .speculative_prefill_target_persistent_state_restored_token_count,
            0,
            "disabled persistent caching must report no sparse target SSD reuse",
        );
        assert_eq!(
            speculative_prefill_measurement
                .speculative_prefill_target_persistent_state_write_count,
            0,
            "disabled persistent caching must publish no sparse target SSD state",
        );
        assert!(
...
pub(super) struct RepresentativePrompt {
    ...
    pub(super) processed_visual_images: Vec<astronomical_model_serving::Qwen3_5ProcessedImage>,
    pub(super) ordinary_target_prefill_control_span_token_count: usize,
    pub(super) sampling_temperature_thousandths: u16,
    pub(super) sampling_top_p_thousandths: u16,
    pub(super) sampling_seed: Option<u64>,
}
