...
#[tokio::test]
#[ignore = "qualifies target-only and protected SpecPrefill tool-call correctness with an 8K Romeo and Juliet prompt"]
async fn should_preserve_a_schema_valid_tool_call_while_fully_prefilling_control_context() {
    tokio::time::timeout(Duration::from_secs(115), async {
        ...
        let validated_target_artifact = Qwen3_5ArtifactValidator::new()
            ...
            .expect("the target artifact should validate for tool qualification");
        let tokenizer = Qwen3_5Tokenizer::from_validated_artifact(&validated_target_artifact)
            .expect("the target tokenizer should load for tool qualification");
        ...
        let persistent_prompt_cache_root_directory = tempfile::tempdir()
            .expect("the tool qualification should create an empty SSD cache root");
    ...
    .await
    .expect("the protected tool-control qualification should finish within 115 seconds");
}

#[tokio::test]
#[ignore = "qualifies the complete tool journey with persistent prompt caching explicitly disabled"]
async fn should_complete_the_tool_journey_without_any_ssd_activity_when_caching_is_disabled() {
    tokio::time::timeout(Duration::from_secs(115), async {
        let _direct_mlx_guard = crate::common::direct_mlx_test_guard().await;
        let target_model_directory = crate::common::configured_ornith_model_artifact_directory();
        let (draft_model_directory, draft_model_id) =
        ...
        let cache_disabled_measurement = run_representative_generation(
            &target_model_directory,
            &draft_model_directory,
            &draft_model_id,
            &representative_tool_prompt,
            ...
            REPRESENTATIVE_TOOL_OUTPUT_TOKEN_COUNT,
            SPECULATIVE_PREFILL_KEEP_PERCENTAGE,
            RequestId::new(95_320),
            None,
            mlx_memory_limits,
        )
        .await;
        let cache_disabled_tool_call = parse_one_tool_call(
            &tokenizer,
            &declared_tools,
            &cache_disabled_measurement.generated_token_ids,
        );
        assert_schema_valid_literary_analysis_tool_call(&cache_disabled_tool_call);
        assert_eq!(
            cache_disabled_measurement
                .speculative_prefill_draft_persistent_prefix_restored_token_count,
            0,
        );
        assert_eq!(
            cache_disabled_measurement.restored_target_persistent_prompt_cache_token_count,
            0,
        );
        assert_eq!(
            cache_disabled_measurement
        ...
        assert_eq!(
            cache_disabled_measurement.speculative_prefill_target_persistent_state_write_count,
            0,
        );
        assert_eq!(
            std::fs::read(&cache_sentinel_path)
                .expect("the cache-disabled sentinel should remain readable"),
            b"unchanged",
        );
        assert_eq!(
            std::fs::read_dir(inaccessible_cache_sentinel_directory.path())
                .expect("the cache-disabled sentinel directory should remain readable")
                .count(),
            1,
            "disabled persistent caching must not create any SSD entry",
        );
        eprintln!("[speculative-prefill-cache-disabled] status=success");
    })
    .await
    .expect("the cache-disabled tool journey should finish within 115 seconds");
}
...
pub(super) fn prepare_representative_tool_prompt(
    ...
    RepresentativePrompt {
        ...
        ordinary_target_prefill_control_span_token_count,
        sampling_temperature_thousandths: if sampling_seed.is_some() { 700 } else { 0 },
        sampling_top_p_thousandths: if sampling_seed.is_some() { 900 } else { 1_000 },
        sampling_seed,
    }
}
...
fn assert_representative_tool_prompt_selection_boundaries(
    ...
    speculative_prefill_measurement: &super::speculative_prefill::RepresentativeGenerationMeasurement,
) {
    ...
    assert!(
        ...
        "the real qualification must exercise sparse rather than full conversation retention",
    );
}
