...
const VISUAL_TOOL_QUALIFICATION_TIMEOUT: Duration = Duration::from_secs(115);
const VISUAL_TOOL_MINIMUM_PROMPT_TOKEN_COUNT: usize = 8_192;
...
#[tokio::test]
#[ignore = "qualifies an image-bearing tool call, mandatory image positions, and changed-image SSD isolation"]
async fn should_preserve_a_visual_tool_call_and_reject_prior_state_for_a_changed_image() {
    tokio::time::timeout(VISUAL_TOOL_QUALIFICATION_TIMEOUT, async {
    ...
    .await
    .expect("the visual tool qualification should finish within 115 seconds");
}

#[tokio::test]
#[ignore = "qualifies ordinary target-only visual processing when the configured compatible drafter is text-only"]
...
fn configured_text_only_compatible_draft(
    ...
    let target_artifact = Qwen3_5ArtifactValidator::new()
        .validate(target_model_directory, 1)
        .expect("the visual target should validate before text-only draft discovery");
    let target_tokenizer_digest = Qwen3_5Tokenizer::token_identifier_mapping_digest(
        target_artifact
            .tokenizer_bytes()
