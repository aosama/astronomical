...
async fn run_one_token_expert_paging_smoke_test() {
    ...
    let (qwen3_5_moe_model, config) = load_paged_model().await;
    let loaded_memory_snapshot = qwen3_5_moe_model
        .runtime()
        .memory_snapshot()
    ...
    let generated_token_id = qwen3_5_moe_model
    ...
    let final_memory_snapshot = qwen3_5_moe_model
        .runtime()
        .memory_snapshot()
...
async fn load_paged_model() -> (Qwen3_5MoEModel, Qwen3_5MoEConfig) {
    let model_directory = super::qwen3_6_35b_a3b_eight_bit_model_directory();
    ...
    let qwen3_5_moe_model =
        Qwen3_5MoEModel::load(runtime, validated_artifact, &model_directory, false).expect(
            "the Qwen3.6-35B-A3B eight-bit model should load with automatic expert residency",
        );
