...
async fn should_attribute_startup_invalidation_once_then_restore_the_rebuilt_cache() {
    ...
    .await
    .expect("the startup-cleanup acceptance should finish within 115 seconds");
...
async fn run_startup_cleanup_attribution_journey() {
    ...
    fs::create_dir_all(&obsolete_sequence_directory)
        .expect("the acceptance should create obsolete format storage");
    ...
    .expect("the acceptance should seed one obsolete format artifact");
