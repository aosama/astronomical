...
async fn should_preserve_visual_prefill_output_across_an_8192_token_cache_restore() {
    ...
    .await
    .expect("the visual prompt-cache acceptance must finish within 115 seconds");
}

#[tokio::test]
#[ignore = "loads Ornith and proves causal prefix reuse when a later image is appended"]
async fn should_restore_the_causal_prefix_before_an_appended_image() {
    timeout(
        VISUAL_ACCEPTANCE_TIMEOUT,
        run_appended_visual_prompt_cache_acceptance(),
    )
    .await
    .expect("the appended-image prompt-cache acceptance must finish within 115 seconds");
}
...
async fn should_restore_visual_and_decoder_state_after_a_checkpoint_prefill_retry() {
    timeout(
        VISUAL_ACCEPTANCE_TIMEOUT,
        run_visual_prompt_cache_acceptance(true),
    )
    .await
    .expect("the visual prefill rollback acceptance must finish within 115 seconds");
}

async fn run_visual_prompt_cache_acceptance(force_prefill_retry: bool) {
    let _direct_mlx_guard = crate::common::direct_mlx_test_guard().await;
    let prompt_cache_directory =
        tempfile::tempdir().expect("the visual acceptance should create a prompt-cache directory");
    let (mut qwen3_5_engine, tokenizer) =
        load_visual_acceptance_engine(prompt_cache_directory.path()).await;
    let cold_request = representative_visual_request(&tokenizer, RequestId::new(9_100));
    assert!(cold_request.input_token_ids().len() >= VISUAL_ACCEPTANCE_MINIMUM_PROMPT_TOKENS);
    qwen3_5_engine
    ...
    if force_prefill_retry {
        qwen3_5_engine
            ...
            .await
            .expect("the visual acceptance should arm one prefill rejection");
    }
    ...
    assert!(
        restored_generation_start.cached_token_count() >= VISUAL_ACCEPTANCE_PREFILL_CHUNK_TOKENS
    );
    ...
    eprintln!(
        "[visual-prompt-cache-acceptance] status=parity cold_token_id={cold_token_id} restored_token_id={restored_token_id} forced_retry={force_prefill_retry}"
    );
    assert_eq!(cold_token_id, restored_token_id);
    eprintln!("[visual-prompt-cache-acceptance] status=success");
}

async fn run_appended_visual_prompt_cache_acceptance() {
    let _direct_mlx_guard = crate::common::direct_mlx_test_guard().await;
    let prompt_cache_directory =
        tempfile::tempdir().expect("the appended-image acceptance should create a cache directory");
    let (mut qwen3_5_engine, tokenizer) =
        load_visual_acceptance_engine(prompt_cache_directory.path()).await;
    ...
    qwen3_5_engine
        ...
        .await
        .expect("the acceptance should clear its private prompt cache")
    ...
    eprintln!(
        "[appended-image-prompt-cache-acceptance] status=success restored_tokens={} token_id={restored_appended_token_id}",
        restored_generation_start.cached_token_count()
    );
}
...
async fn load_visual_acceptance_engine(
    ...
    qwen3_5_engine
        .load()
        .await
        .expect("the visual acceptance engine should load");
...
fn representative_visual_request_with_optional_later_image(
    ...
    for context_sentence_repetition_count in (512..=2_048).step_by(128) {
        ...
        if request.input_token_ids().len() >= VISUAL_ACCEPTANCE_MINIMUM_PROMPT_TOKENS {
            return request;
        }
    }
    ...
    loop {
            ...
            .expect("the visual request should advance")
        {
            ...
            GeneratedToken::PrefillProgress {
                ...
                eprintln!(
                    "[visual-prompt-cache-acceptance] completed_prefill_chunk_tokens={completed_prefill_chunk_tokens} active_memory_bytes={:?} peak_memory_bytes={:?}",
                    mlx_memory_telemetry
                        .as_ref()
...
async fn prompt_cache_sequence_state_block_count(qwen3_5_engine: &Qwen3_5Engine) -> u64 {
    let cache_stats = qwen3_5_engine
        ...
        .await
        .expect("the visual acceptance should collect cache stats")
        .expect("the visual acceptance should have a prompt cache");
    let WorkerEvent::PersistentPromptCacheStats {
        persistent_prompt_cache_sequence_state_block_count,
        ..
    } = cache_stats
    else {
        panic!("the visual acceptance received unexpected cache stats")
    };
    persistent_prompt_cache_sequence_state_block_count
}
