...
#[test]
fn should_parse_the_complete_optiq_mixed_precision_quantization_map() {
    let config_bytes = certified_optiq_ornith_config_bytes();

    let ornith_config = Qwen3_5Config::from_json_bytes(&config_bytes)
        .expect("the certified OptiQ Ornith config should parse");
    let quantized_module_profiles = ornith_config.quantized_module_profiles();

    assert_eq!(ornith_config.activation_dtype(), "bfloat16");
    assert_eq!(quantized_module_profiles.len(), 512);
    assert_eq!(
        quantized_module_profiles
            .values()
            .filter(|profile| profile.bits == 4)
            .count(),
        113
    );
    assert_eq!(
    ...
    // All modules should have group_size 64 in the oQ4 model
    assert!(
        quantized_module_profiles
            .values()
            .all(|profile| profile.group_size == 64)
    );
}
