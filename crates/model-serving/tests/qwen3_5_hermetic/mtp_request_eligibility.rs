...
fn should_keep_ssd_paged_sparse_experts_target_only() {
    assert!(!qwen3_5_mtp_request_is_eligible(
        true, true, true, true, false, false, false, true, 32, 0,
    ));
}
...
fn should_prefer_persistent_prompt_cache_over_multi_token_prediction() {
    assert!(!qwen3_5_mtp_request_is_eligible(
        true, true, true, true, false, false, true, false, 32, 0,
    ));
}

#[test]
fn should_keep_sampled_requests_target_only() {
    assert!(!qwen3_5_mtp_request_is_eligible(
        true, true, true, false, false, false, false, false, 32, 0,
    ));
}
