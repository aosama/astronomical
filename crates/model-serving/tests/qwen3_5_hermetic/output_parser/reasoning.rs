...
#[test]
fn should_emit_reasoning_and_text_without_qwen3_5_marker_syntax() {
    let mut output_parser = Qwen3_5OutputParser::new(&[])
    ...
    assert_eq!(
        ...
        vec![Qwen3_5OutputEvent::ReasoningDelta(
    ...
    assert_eq!(
        ...
        vec![Qwen3_5OutputEvent::TextDelta("then edit it.".to_owned())]
    );
    assert!(
        output_parser
            .finish()
...
fn should_continue_reasoning_already_opened_by_the_generation_prompt() {
    let mut output_parser = Qwen3_5OutputParser::new_after_thinking_prefix(&[])
    ...
    assert_eq!(
        ...
        vec![
            Qwen3_5OutputEvent::ReasoningDelta("Inspect first".to_owned()),
            Qwen3_5OutputEvent::TextDelta("Then answer.".to_owned()),
        ]
    );
}
...
fn should_suppress_late_thinking_blocks_after_visible_text_has_started() {
    ...
    assert_eq!(
        ...
        vec![
            Qwen3_5OutputEvent::ReasoningDelta("Initial plan".to_owned()),
            Qwen3_5OutputEvent::TextDelta("Visible answer.".to_owned()),
            Qwen3_5OutputEvent::TextDelta("Done.".to_owned()),
        ]
    );
}
...
fn should_buffer_control_markers_split_across_token_fragments() {
    ...
    assert_eq!(
        output_parser
            ...
            .as_slice(),
        [Qwen3_5OutputEvent::ReasoningDelta("plan".to_owned())]
    );
    assert_eq!(
        output_parser
            .push_fragment("nk>answer")
            .expect("the completed thinking-end marker should reveal text"),
        vec![Qwen3_5OutputEvent::TextDelta("answer".to_owned())]
    );
}
