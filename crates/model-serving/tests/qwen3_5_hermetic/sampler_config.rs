...
fn should_use_temperature_top_p_and_top_k_from_model_generation_configuration() {
    ...
    assert_eq!(model_sampler_configuration.certified_top_k, 20);
}
...
#[test]
fn should_preserve_a_model_declared_zero_temperature_as_greedy_sampling() {
    // A zero value is a model policy, not a missing field. This protects real-artifact
    // qualification from silently changing a deterministic model into randomized sampling.
    ...
    assert_eq!(model_sampler_configuration.certified_top_k, 40);
}
