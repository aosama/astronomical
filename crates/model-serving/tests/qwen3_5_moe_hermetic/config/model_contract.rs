...
fn should_parse_a_native_bfloat16_config_without_quantization_metadata() {
    ...
    let native_bfloat16_config = Qwen3_5MoEConfig::from_json_bytes(&native_bfloat16_config_bytes)
    ...
    let native_bfloat16_tensor_profiles =
        qwen3_5_moe_language_tensor_profiles(&native_bfloat16_config);
    assert!(
...
fn should_estimate_long_context_memory_from_full_attention_key_value_state_only() {
    let ornith_config = Qwen3_5MoEConfig::from_json_bytes(&certified_ornith_config_bytes())
        .expect("the certified Ornith core config should parse");
...
fn should_reject_an_ornith_layer_schedule_with_the_wrong_number_of_layers() {
    ...
    assert!(matches!(
        ...
        Err(Qwen3_5MoEConfigError::LayerTypeCountMismatch {
...
fn should_reject_linear_attention_dimensions_that_exceed_the_mlx_shape_range() {
    ...
    assert!(matches!(
        Qwen3_5MoEConfig::from_json_bytes(&config_bytes),
        Err(Qwen3_5MoEConfigError::InvalidConfigValue { .. })
