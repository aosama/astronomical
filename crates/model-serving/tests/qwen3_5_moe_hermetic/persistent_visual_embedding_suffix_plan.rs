use astronomical_model_serving::{
    Qwen3_5MoEVisualEmbeddingRequiredImage, plan_qwen3_5_moe_visual_embedding_suffix,
};
...
fn should_use_the_caller_supplied_image_pad_token_id() {
    ...
    let suffix_plan = plan_qwen3_5_moe_visual_embedding_suffix(
        &prompt_token_ids,
        1,
        &[2],
        caller_image_pad_token_id,
    )
    ...
    assert_eq!(
        ...
        &[Qwen3_5MoEVisualEmbeddingRequiredImage::new(0, 2, 1, 1)]
    );
}
...
fn should_require_every_image_when_no_prompt_tokens_were_restored() {
    ...
    let suffix_plan =
        plan_qwen3_5_moe_visual_embedding_suffix(&prompt_token_ids, 0, &[2, 3], IMAGE_PAD_TOKEN_ID)
    ...
    assert_eq!(
        suffix_plan.required_images(),
        &[
            Qwen3_5MoEVisualEmbeddingRequiredImage::new(0, 2, 0, 2),
            Qwen3_5MoEVisualEmbeddingRequiredImage::new(1, 3, 0, 3),
        ]
    );
}
...
fn should_skip_images_fully_covered_by_restored_prompt_state() {
    ...
    let suffix_plan =
        plan_qwen3_5_moe_visual_embedding_suffix(&prompt_token_ids, 3, &[2, 3], IMAGE_PAD_TOKEN_ID)
            .expect("the visual suffix plan should be valid");
...
fn should_start_inside_the_first_remaining_image_when_restore_splits_an_image_pad_run() {
    ...
    let suffix_plan =
        plan_qwen3_5_moe_visual_embedding_suffix(&prompt_token_ids, 1, &[3, 2], IMAGE_PAD_TOKEN_ID)
            .expect("the visual suffix plan should be valid");
    ...
    assert_eq!(
        suffix_plan.required_images(),
        &[
            Qwen3_5MoEVisualEmbeddingRequiredImage::new(0, 3, 1, 2),
            Qwen3_5MoEVisualEmbeddingRequiredImage::new(1, 2, 0, 2),
        ]
    );
}
...
fn should_require_no_images_when_restored_prompt_state_covers_every_image_pad_token() {
    ...
    let suffix_plan =
        plan_qwen3_5_moe_visual_embedding_suffix(&prompt_token_ids, 5, &[2, 2], IMAGE_PAD_TOKEN_ID)
            .expect("the visual suffix plan should be valid");
...
fn should_reject_a_prompt_whose_image_pad_count_does_not_match_image_rows() {
    ...
    let suffix_plan =
        plan_qwen3_5_moe_visual_embedding_suffix(&prompt_token_ids, 0, &[1], IMAGE_PAD_TOKEN_ID);
