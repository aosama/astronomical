use astronomical_model_serving::{
    Qwen3_5MoEImageProcessor, Qwen3_5MoETokenizer, Qwen3_5MoETokenizerError,
...
fn should_discover_special_token_ids_from_tokenizer_json() {
    let tokenizer = Qwen3_5MoETokenizer::from_json_bytes(
        &ornith_tokenizer_json_bytes(248_056),
        ORNITH_VOCABULARY_SIZE,
        ORNITH_MAXIMUM_POSITION_COUNT,
        Qwen3_5MoEImageProcessor::qwen3_5_moe_35b_optiq(),
    )
...
fn should_reject_a_tokenizer_missing_a_required_special_token() {
    let tokenizer_error = Qwen3_5MoETokenizer::from_json_bytes(
    ...
    assert!(matches!(
        tokenizer_error,
        Qwen3_5MoETokenizerError::DiscoverTokenIds { .. }
    ));
}
