...
pub(super) fn installed_model_has_complete_mtp_inventory(model_directory: &Path) -> bool {
    Qwen3_5ArtifactValidator::new()
        .validate(model_directory, 20_480)
        .map(|validated_artifact| {
            validated_artifact
...
pub(super) async fn generate_with_mtp_engine(
    ...
    qwen3_5_engine
        ...
        .await
        .expect("the engine should accept one MTP generation request");
...
pub(super) fn configured_mtp_artifact_test_inputs(
    ...
    let short_prompt_request = tokenizer
        ...
        .expect("the configured MTP tokenizer should prepare the short acceptance prompt");
