...
#[tokio::test]
#[ignore = "loads the local MTP artifact twice and proves injection plus terminal lifecycle boundaries"]
async fn should_preserve_active_mtp_across_injection_and_request_lifecycle_boundaries() {
    tokio::time::timeout(Duration::from_secs(115), run_mtp_lifecycle_acceptance())
        .await
        .expect("the MTP lifecycle acceptance should finish within 115 seconds");
}

async fn run_mtp_lifecycle_acceptance() {
    let _direct_mlx_guard = crate::common::direct_mlx_test_guard().await;
    let model_directory =
        crate::serving_acceptance::support::configured_depth_one_mtp_model_directory();
    if !super::engine_support::installed_model_has_complete_mtp_inventory(&model_directory) {
        eprintln!(
            "[mtp-lifecycle] status=success phase=target_only reason=installed_model_has_no_complete_mtp_inventory"
        );
        return;
    }
    ...
    eprintln!(
        "[mtp-lifecycle] status=diagnostic phase=injection exact_highest_logit_match={} first_highest_logit_mismatch={first_injected_continuation_mismatch:?}",
...
async fn start_generation_request(
    engine: &mut Qwen3_5Engine,
    request_id: RequestId,
    short_prompt_token_ids: &[u32],
    image_pad_token_id: u32,
    ...
    engine
        ...
        .await
        .expect("the reused engine should accept the generation request");
}
