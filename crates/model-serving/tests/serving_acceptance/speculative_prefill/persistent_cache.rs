...
async fn should_reuse_target_and_drafter_ssd_state_for_a_chat_follow_up() {
    tokio::time::timeout(Duration::from_secs(115), async {
        ...
        let initial_prompt = super::RepresentativePrompt {
            prompt_token_ids: representative_source_prompt.prompt_token_ids.clone(),
        ...
        let follow_up_prompt = super::RepresentativePrompt {
        ...
        let target_artifact = Qwen3_5ArtifactValidator::new()
            ...
            .expect("the target artifact should validate for continuation acceptance");
        assert!(
    ...
    .await
    .expect("the drafter SSD acceptance should finish within 115 seconds");
}
...
fn assert_long_chat_follow_up_session_preserves_speculative_prefill_rollback_state(
    ...
    const MINIMUM_RESTORED_DRAFTER_PREFIX_TOKEN_COUNT: u64 = 1;
    const FULL_FOLLOW_UP_PROMPT_TOKEN_COUNT: u64 =
        (INITIAL_CHAT_PROMPT_TOKEN_COUNT + FOLLOW_UP_MESSAGE_TOKEN_COUNT) as u64;
    ...
    assert!(
        follow_up_measurement.speculative_prefill_draft_persistent_prefix_restored_token_count
            >= MINIMUM_RESTORED_DRAFTER_PREFIX_TOKEN_COUNT,
        "the follow-up should restore dense drafter prefix from the cold SpecPrefill pass"
    );
    assert!(
        follow_up_measurement.speculative_prefill_target_persistent_state_restored_token_count
    ...
    assert!(
        follow_up_measurement.speculative_prefill_draft_scored_suffix_token_count > ZERO_THRESHOLD,
        "the drafter must score the uncached follow-up suffix"
    );
    assert!(
        follow_up_measurement.speculative_prefill_draft_scored_suffix_token_count
            < FULL_FOLLOW_UP_PROMPT_TOKEN_COUNT,
        "the follow-up must restore some dense drafter prefix instead of rescoring the whole chat"
    );
