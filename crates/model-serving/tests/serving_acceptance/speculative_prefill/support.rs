...
/// Builds a Romeo and Juliet summary prompt without a thinking reservation.
/// Paging and residency identity tests use tiny output caps; a 256-token thinking budget would fail closed on those caps.
...
pub(crate) fn prepare_representative_prompt(model_directory: &Path) -> RepresentativePrompt {
    let validated_artifact = Qwen3_5ArtifactValidator::new()
