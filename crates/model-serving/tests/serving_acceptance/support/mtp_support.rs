...
use crate::serving_acceptance::support::SAY_HI_PROMPT_TOKEN_IDS;

pub(crate) async fn run_one_layer_mtp_head_forward_acceptance(
    ...
    let validated_artifact = Qwen3_5ArtifactValidator::new()
        ...
        .expect("the local MTP artifact should validate before native loading");
    if !validated_artifact
        .mtp_artifact_capability()
        .is_mtp_capable()
    {
        eprintln!(
            "[{progress_log_prefix}] status=success phase=target_only reason=installed_model_has_no_complete_mtp_inventory"
        );
        return;
    }
    ...
    let next_token_id = qwen3_5_model
        .highest_logit_token_id(target_forward_output.final_logits())
