...
fn should_default_the_image_count_and_optional_seed() {
    let request = deserialize_request(&valid_request_fields("64", "64"));
...
#[test]
fn should_reject_step_and_guidance_fields_as_unknown() {
    ...
    for intruding_field in ["steps", "guidance"] {
        let request_json = format!(
            r#"{{"model":"flux","prompt":"A rose.","width":64,"height":64,"response_format":"b64_json","{intruding_field}":1}}"#
        );
        assert_eq!(
            deserialize_request(&request_json).into_parts(),
            Err(OpenAiImageGenerationValidationError::UnknownField {
                field_name: intruding_field.to_owned(),
            })
        );
    }
}
...
fn should_reject_blank_or_non_string_prompts() {
    ...
    assert_malformed_request(
        r#"{"model":"flux","prompt":7,"width":64,"height":64,"response_format":"b64_json"}"#,
    );
}
...
fn should_reject_dimensions_outside_the_supported_geometry() {
    ...
    for (width, height, parameter_name, actual_pixels) in invalid_dimensions {
        let request = deserialize_request(&valid_request_fields(width, height));
        assert_eq!(
            request.into_parts(),
...
#[test]
fn should_reject_malformed_dimension_transport_values() {
    for malformed_fields in [
        valid_request_fields("64.5", "64"),
        valid_request_fields("-64", "64"),
        valid_request_fields("4294967296", "64"),
    ] {
...
#[test]
fn should_reject_malformed_seed_transport_values() {
    for malformed_seed in ["-1", "1.5", "18446744073709551616", "\"7\""] {
        let request_json = format!(
            r#"{{"model":"flux","prompt":"A rose.","seed":{malformed_seed},"width":64,"height":64,"response_format":"b64_json"}}"#
        );
...
fn should_reject_unsupported_format_count_and_unknown_fields() {
    let invalid_requests = [
        (
            r#"{"model":"flux","prompt":"A rose.","width":64,"height":64,"response_format":"url"}"#,
            OpenAiImageGenerationValidationError::UnsupportedResponseFormat {
                response_format: "url".to_owned(),
            },
        ),
        (
            r#"{"model":"flux","prompt":"A rose.","width":64,"height":64,"response_format":"b64_json","n":2}"#,
            OpenAiImageGenerationValidationError::UnsupportedImageCount { actual_images: 2 },
        ),
        (
            r#"{"model":"flux","prompt":"A rose.","width":64,"height":64,"response_format":"b64_json","quality":"hd"}"#,
...
fn valid_request_fields(width: &str, height: &str) -> String {
    format!(
        r#"{{"model":"flux","prompt":"A rose.","width":{width},"height":{height},"response_format":"b64_json"}}"#
    )
}
