...
void should_release_native_transaction_allocations_between_repetitions() {
  std::cout
      ...
      << std::endl;
  TemporaryDirectory temporary_directory;
  const auto source_pack_path = temporary_directory.path() / "repeated.expert-pack";
  write_synthetic_pack(source_pack_path);
  NativeStreamOwner gpu_stream;
  const std::vector<int> output_shape = {
      static_cast<int>(kSyntheticPackByteCount / sizeof(uint32_t))};
  ...
  for (size_t repetition_index = 0; repetition_index < 4; ++repetition_index) {
    CompleteTransactionOwner transaction(
        source_pack_path,
        output_tensors,
        load_ranges,
...
void should_repeat_large_eight_range_native_transactions() {
  ...
  TemporaryDirectory temporary_directory;
  const auto source_pack_path = temporary_directory.path() / "large-eight-range.expert-pack";
  ...
  std::ofstream source_pack_file(source_pack_path, std::ios::binary | std::ios::trunc);
  ...
  require_condition(source_pack_file.good(), "could not write the large native test pack");
  NativeStreamOwner gpu_stream;
  const std::vector<int> output_shape = {
