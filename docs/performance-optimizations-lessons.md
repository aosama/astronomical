...
- MLX safetensors arrays are lazy: page construction records Load primitives, while positional SSD reads run later on MLX input/output workers during evaluation. Measure exact reader-callback call count, bytes, total worker read time, maximum read latency, and failures without adding an evaluation boundary. Keep the synchronization wait as wall time and do not subtract summed worker time from it because concurrent reads can overlap and the same wait can include queued computation and thread-pool delay.
...
- Direct Metal input/output benefits grow with expert-page payload while tiny pages can be effectively tied with positional reads. Measure representative decode and prefill expert counts before adopting a storage path. Do not redesign the disk format solely to reduce command count when host encoding remains small; first prove whether one contiguous expert record improves queue time enough to justify strided views or kernel changes.
- An expert-major disk record can concatenate weight, scales, and biases for all projections without extra padding when the natural record size is already a multiple of the required storage alignment. Keep conversion cost out of request handling and validate every field byte before evaluating a one-read-per-expert design.
- Storage-layout comparisons must make the graphics processor consume every loaded output. Consuming only the first output gives unequal work when one layout returns many field arrays and another returns one complete record, invalidating complete-transaction attribution.
- Fewer Metal input/output commands and lower host encoding time do not establish an end-to-end win. Require representative expert widths to improve complete transactions consistently before adding a production format, repacking kernel, or paging branch.
- Measure prompt processing and token generation as separate expert-loading workloads. Prompt processing combines many routed rows and broad expert coverage, while generation combines one token with sparse top-k routing; one aggregate number can hide opposite storage behavior.
- Keep storage-path timing at the file-load plus immediate gather-QMM boundary. Build test inputs before timing and copy outputs to the host only afterward, so orchestration and parity checks do not obscure the data-plane result.
- Validate prepared pack identity, layer geometry, source file names and sizes, tensor shapes, dtypes, and quantization at pager startup. Request-time expert loading should only calculate selected offsets and submit Metal input/output commands; mismatched or absent packs should use the existing bounded reader.
- Keep model repacking outside model loading and request handling. Require one explicit command for an already-downloaded model, preflight the exact remaining disk requirement, lock one revision, build resumably in staging, validate every layer, then rename the complete revision atomically.
- Prove prepared payload integrity before publication by comparing every logical packed tensor segment with its source bytes. Persist source size and modification identity in the format header so startup can reject stale packs without hashing tens of gigabytes.
- Activate prepared expert packs as one complete revision or not at all. A layer-by-layer mixture of prepared and source formats adds request-path states while providing no useful recovery advantage over one startup warning and bounded-reader fallback.
- Attribute aligned Metal input/output separately from lazy bounded-safetensors page construction. Record transaction count and logical payload bytes so storage-path conclusions do not confuse graph construction with native file transfer.
...
- Keep sequential reader cursor state separate from immutable bounded-range mappings. Positional read_at_offset callbacks use independent file offsets and must not acquire the cursor mutex; otherwise one lock serializes the Machine Learning framework for Apple silicon input/output worker pool.
