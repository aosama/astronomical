...
- Serialize persistent prompt-cache tensors while they remain on the Machine Learning framework for Apple silicon owner thread, then transfer only bounded byte vectors to a filesystem thread. The default writer should use the device and operating-system throughput rather than an arbitrary fixed rate; retain an explicit user cap for machines that need one. Reject a capture that cannot fit the serialized-byte bound, but do not drop a valid successor merely because the writer still owns its parent. A zero-capacity handoff bounds this ordered path to the writer-owned block plus one producer-owned successor and applies backpressure without another scheduler or a larger machine-specific allowance.
...
- For one large prefill that crosses several persistent prompt-cache boundaries, compare one model-derived block with the pending serialized-memory bound because blocks serialize and hand off sequentially. Compare every crossed block with the disk quota because all published files remain resident. Multiplying all blocks against pending memory rejects safe large forwards; checking only one block against disk can exceed storage admission. A buffered non-blocking channel can still reject the third production-sized boundary while two serialized blocks remain in flight, so use a rendezvous handoff that waits for writer ownership to advance before serializing the next successor.
- Once one prompt-cache boundary is skipped because the writer cannot accept it, stop capture for that request. Resuming after a gap creates an unusable parent chain and can reclaim the newest useful recurrent snapshot even though generation remains correct.
...
- Persistent prompt-cache block size must not cap model-forward size. Capture exact intermediate convolution and recurrent state during one large forward, persist fixed-size blocks in prompt order, and record the complete forward as the optimizer observation.
