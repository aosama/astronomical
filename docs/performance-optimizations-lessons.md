...
- Attribute aligned Metal input/output separately from lazy bounded-safetensors page construction. Record submission bytes, command count, and native host-encoding time immediately, then let the command-buffer completion handler record queue time and completion status into lock-free counters. Do not add a host completion wait to the dependent graphics-processor path solely for attribution.
