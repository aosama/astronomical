...
pub(super) use e2e_test_model_names::large_sparse_moe_model_id;
...
fn configured_model_directory_by_id(model_id: &str) -> Option<PathBuf> {
    AstronomicalConfig::load_from_development_location()
        .expect("the standard Astronomical configuration should load for experiment acceptance")
...
async fn require_aligned_expert_pack_completion(test_future: impl Future<Output = ()>) {
    ...
    loop {
        tokio::select! {
            ...
            () = &mut timeout_deadline => panic!("the experimental aligned expert-pack acceptance exceeded {} seconds", ALIGNED_EXPERT_PACK_TEST_TIMEOUT.as_secs()),
