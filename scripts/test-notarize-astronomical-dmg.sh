...
XCRUN
    ...
    FAKE_NOTARY_LOG="$notary_log" PATH="${fake_bin}:${PATH}" timeout "$SUBJECT_TIMEOUT_SECONDS" \
        "${repository_root}/scripts/notarize-astronomical-dmg.sh" --dmg "$fixture_dmg" \
    ...
    if FAKE_NOTARY_REJECT=true FAKE_NOTARY_LOG="$notary_log" PATH="${fake_bin}:${PATH}" \
        timeout "$SUBJECT_TIMEOUT_SECONDS" "${repository_root}/scripts/notarize-astronomical-dmg.sh" \
